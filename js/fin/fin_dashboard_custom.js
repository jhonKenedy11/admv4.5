...
function init_flot_chart(data) {
    ...
    if ($("#chart_plot_01").length) {
        ...
        var totalPerDebElement = document.getElementById('total_periodo_debito');
        if (totalPerDebElement) {
            var valorFormatado = parseFloat(data.total_periodo_debito).toLocaleString('pt-BR', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2
            });
            totalPerDebElement.innerText = valorFormatado;
        }

        var totalPerCredElement = document.getElementById('total_periodo_credito');
        if (totalPerCredElement) {
            var valorFormatado = parseFloat(data.total_periodo_credito).toLocaleString('pt-BR', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2
            });
            totalPerCredElement.innerText = valorFormatado;
        }
    }
...
function init_chart_doughnut(data) {
    ...
    bancos.forEach(function (banco, index) {
        ...
        // Adiciona a classe 'col-md-4' a cada célula
        cell1.classList.add('tdNomeContas');
        cell2.classList.add('tdSaldoContas');
        cell3.classList.add('tdNomeContas');
    
        cell1.innerHTML = '<p class="nameContas"><i class="fa fa-square" style="color:' + data.datasets.backgroundColor[index] + '"></i>' + banco + '</p>';
        cell2.innerHTML = '<p class="saldoContas text-center"> R$ ' + formatarValor(data.saldos[index]) + '</p>';
        cell3.innerHTML = '<p class="percContas text-center">' + data.dataTabela[index] + '% </p>';
    });
...
function init_gauge_vel(data) {
    ...
    if ($('#gauge-text').length) {
        ...
        chart_gauge_01.animationSpeed = 32;
        chart_gauge_01.set(data.percentual_debito_baixado);
        chart_gauge_01.setTextField(document.getElementById("gauge-text"));
    }
    if ($('#chart_gauge_02').length) {
        var chart_gauge_02_elem = document.getElementById('chart_gauge_02');
    ...
    if ($('#gauge-text2').length) {
        ...
        chart_gauge_02.animationSpeed = 32;
        chart_gauge_02.set(data.percentual_credito_baixado);
        chart_gauge_02.setTextField(document.getElementById("gauge-text2"));
    }
    debugger
    // left-debito-baixado
    var leftDebitoBaixado = document.getElementById('left-debito-baixado');
    if (leftDebitoBaixado) {
        console.log("Setting value for #left-debito-baixado");
        leftDebitoBaixado.innerText = "R$ " + formatarValor(data.valor_debito_baixado);
    } else {
        console.error("#left-debito-baixado element not found");
    }
    // right-debito-aberto
    var rightDebitoAberto = document.getElementById('right-debito-aberto');
    if (rightDebitoAberto) {
        console.log("Setting value for #right-debito-baixado");
        rightDebitoAberto.innerText = "R$ " + formatarValor(data.valor_debito_aberto);
    } else {
        console.error("#right-debito-aberto element not found");
    }
    // left-credito-baixado
    var leftCreditoBaixado = document.getElementById('left-credito-baixado');
    if (leftCreditoBaixado) {
        console.log("Setting value for #left-credito-baixado");
        leftCreditoBaixado.innerText = "R$ " + formatarValor(data.valor_credito_baixado);
    } else {
        console.error("#left-credito-baixado element not found");
    }
    // right-credito-aberto
    var rightCreditoAberto = document.getElementById('right-credito-aberto');
    if (rightCreditoAberto) {
        console.log("Setting value for #rigth-credito-aberto");
        rightCreditoAberto.innerText = "R$ " + formatarValor(data.valor_credito_aberto);
    } else {
        console.error("#right-credito-aberto element not found");
    }
};
