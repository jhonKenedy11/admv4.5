$(document).ready(function() {
    ...
    $('#cliente').select2({
        ...
        ajax: {
            dataType: "json",
            type: "POST",
			url: window.location.pathname + '?mod=cat&form=faturamento_nfs&submenu=pesquisaClienteAjax&opcao=ajax',
    ...
    const removeClass = document.querySelector("#checkTodos");
    if (removeClass) {
...
function searchDocuments() {
	...
	try {
		...
		$.ajax({
			url: window.location.pathname + '?mod=cat&form=faturamento_nfs&submenu=searchDocuments&opcao=ajax',
			type: 'POST',
			dataType: 'json',
    ...
    dados.forEach(function(item) {
        ...
        var row = '<tr>' +
            ...
            '<td class="text-center">' +
                ...
                '<button class="btn btn-xs btn-info" title="Ver Serviços" onclick="abrirModalServicos(' + id + ', \'' + tipoDocumento + '\', event)"><i class="fa fa-list"></i></button> ' +
