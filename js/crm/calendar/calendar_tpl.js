    ...
    let calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        header: {
          ...
          datesRender: function(info) {
            ...
            //ajax que busca data bloqueada para entrega
            let data = info.view.currentStart;
            data = data.toLocaleDateString('en-CA')
            $.ajax({
                url: document.URL + "mod=crm&form=calendar&submenu=busca_datas_bloqueadas&opcao=blank",
                ...
                success: function (e) {
                    e.forEach(function(data) {
                        if(data){
                            var celula = document.querySelector('.fc-day[data-date="' + data.data + '"]');
                            celula.style.backgroundColor = 'rgb(255 207 207)';
                        }
                    });
                },
                error: function (e) {
                    
                    alert('Erro ao processar events: ' + e.responseText);
                }
            });
            //fim ajax data bloqueada
        },
        customButtons: {
            myCustomButton: {
...
//Busca as data bloqueadas para entrega
document.addEventListener('DOMContentLoaded', function (calendar) {
    
    let data = document.lastModified;
    ...
    $.ajax({
        ...
        error: function (e) {
            ...
            alert('Erro ao processar as datas bloqueadas: ' + response.responseText);
        }
    });
});
...
function submitUpdateReg() {
    ...
    let proximo_contato = $('#proximoContato').val();
...
function trataData(data){
    let string_split = data.split("-");

    let ano = string_split[0];
    let mes = string_split[1];
    let dia = string_split[2];
