...
function editarPeca(e, nrItem){
    ...
    var codigoProduto = linha.find("td:eq(1)").text().trim(); 
    ...
    var totalitem = linha.find("td:eq(9)").text().trim();
...
function limpaCamposServicos(){
    ...
    document.lancamento.totalServico.value = ''
    document.lancamento.obsItemServico.value = ''
    document.getElementById('collapseInfoAdicional').classList.remove('in');
}
...
function buscaProduto() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            // Garante que o evento de atualização do desconto funcione mesmo com a máscara
            $("#valorDesconto").off('change input').on('change input', function() {
                atualizarInfo();
            });
        }
    });
  ...
  function printRomaneio(id) {
    ...
    $( "#popup" ).dialog({
        ...
        "Loja": function() {
            ...
            $( this ).dialog( "close" );
            window.open("index.php?mod=ped&form=rel_pedido_ps&letra=loja&opcao=imprimir&id="+id,
        "toolbar=no,location=no,resizable=yes,menubar=yes,width=950,height=900,scrollbars=yes");
        },
        "Cliente": function() {
            $( this ).dialog( "close" );
            window.open("index.php?mod=ped&form=rel_pedido_ps&letra=cliente&opcao=imprimir&letra=cliente&id="+id,
        "toolbar=no,location=no,resizable=yes,menubar=yes,width=950,height=900,scrollbars=yes");
        }
        }
    });
...
function submitVendaPerdida(id) {
