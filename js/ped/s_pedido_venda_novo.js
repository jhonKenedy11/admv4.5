...
async function submitConfirmarSmart(pathCliente = '') {
  debugger;
...
function submitConfirmarCotacao() {
  debugger;
  ...
  if (confirm("Deseja realmente salvar esta Cotação?") == true) {
    f.submenu.value = "alteraCotacao";
    f.submit();
  } 
  
} // submitConfirmarCotacao
...
function abrir(pag, form='') {
  ...
  window.open(
    pag,
    "consulta",
    "toolbar=no,location=no,menubar=no,width="+screenWidth+",height="+screenHeight+",scrollbars=yes"
  );
}
    ...
    if (f.natop != undefined && f.natop.value == "") {
      ...
      if (f.condPgto.value == "0") {
        ...
        for (i = 1; i < r; i++) {
          ...
          if(f.pesquisa_prod_vazio.value == 'true'){
        ...
        "|" + "|" +
        f.pesLocalizacao.value;
        f.form.value ='pedido_venda_telhas_novo'
        f.submenu.value = "cadastrarItem";
        // p/ nao perder as casas decimais ex: 10,50
        var newFrete = parseFloat(f.frete.value.replace(".", "").replace(",", "."))
        f.frete.value = newFrete;
...
function calculaPerc(campo) {
  debugger
  var f = document.lancamento;
  //var totalItem     = f.totalitem.value;
  var desconto      = campo != 'desconto' ? desconto = "0,00" : desconto =  f.descontoItem.value;
  var percDesconto  = campo == 'desconto' ? percDesconto = "0,00" : percDesconto = f.percDescontoItem.value;
  var qtde          = f.quantidade.value;
  var vlrUnitario   = f.unitario.value; 
  ...
  totalItem  = (vlrUnitario * qtde);

  if(campo == 'desconto'){
    percDesconto = ((desconto * 100)/totalItem)
  }else{
    desconto = ((totalItem*percDesconto)/100)
  }

  totalItemDesconto = totalItem - desconto;

  resultTotal = totalItem;
  resultPerc = currencyFormat(percDesconto);
  resultDesc = currencyFormat(desconto);
  total = currencyFormat(resultTotal);

  totalItemDesconto = currencyFormat(totalItemDesconto);


  if (resultPerc === "NaN" || resultDesc === "NaN"){
      f.descontoItem.value = 0;
      f.percDescontoItem.value = 0;
  }else if(resultPerc === "Infinity" || resultDesc === "Infinity"){
    f.descontoItem.value = 0;
    f.percDescontoItem.value = 0;
  }else{
      f.descontoItem.value = resultDesc;
      f.percDescontoItem.value = resultPerc;
      f.totalItem.value = total
      f.totalitemDesconto.value = totalItemDesconto
  }
}

function confirmacaoDesconto(){
  f = document.lancamento;
  if(f.desconto.value != '0' || f.desconto.value != ''){
...
function pesquisaProdutoVazio(id, subMenu) {
  debugger
  f = document.lancamento;  
  f.submenu.value = subMenu;
  f.id.value = id 
  f.pesquisa_prod_vazio.value = true;
  f.submit()
...
function printRomaneio(id) {
  $( "#popup" ).dialog({
    ...
    buttons: {
      "Loja": function() {
        $( this ).dialog( "close" );
        window.open("index.php?mod=ped&form=pedido_venda_imp_romaneio&opcao=imprimir&parm="+id,
      "toolbar=no,location=no,resizable=yes,menubar=yes,width=950,height=900,scrollbars=yes");
      },
      "Cliente": function() {
        $( this ).dialog( "close" );
        window.open("index.php?mod=ped&form=pedido_venda_imp_romaneio&opcao=imprimir&letra=cliente&parm="+id,
      "toolbar=no,location=no,resizable=yes,menubar=yes,width=950,height=900,scrollbars=yes");
      }
    }
  });
