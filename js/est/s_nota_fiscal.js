...
function submitLetra() {
    ...
    if(f.from.value === 'manifesto_fiscal'){
        f.opcao.value = 'imprimir';
        f.from.value = 'manifesto_fiscal';
    }
    montaLetra();
    f.submit();
}// submitLetra
...
function submitVoltarNfMostra(id) {
    window.location.href = 'index.php?mod=est&form=nota_fiscal&submenu=mostra';
} // fim submitVoltar
...
function addParcelasFinanceiro(id) {
     
    f = document.lancamento;
    f.mod.value = 'ped';
    f.form.value = 'pedido_venda_nf_pecas_novo';
    f.opcao.value = '';
    f.submenu.value = 'financeiroEntradaNf';
    f.id.value = id;
    f.t_origem.value = 'nota_fiscal';
    f.submit();
}

//fecha pesquisa de nota fiscal e atualiza campos da form que chamou
function fechaPesquisaParamManifesto(idNotaFiscal, numNotaFiscal, descPessoa, numPedido, descfilial, idMdf, origemNfe) {
     
    if(origemNfe === 'PED'){
        f = window.opener.document.lancamento;
        if(idMdf !== f.idMdf.value){
            f.idNotaFiscal.value = idNotaFiscal;
            f.numNotaFiscal.value = numNotaFiscal;
            f.numPedido.value = numPedido;
            f.descPessoa.value = descPessoa;
            f.filial.value = descfilial;
            
            window.close();
        }else{
             swal.fire({
                title:"Atenção!",
                text:"Nota já existe no manifesto Nº " + idMdf + ".",
                icon:"warning"});
        }
    }else{
         swal.fire({
            title:"Atenção!",
            text:"Selecione uma NFe Válida!",
            icon:"warning"});
    }
}

function submitConsultaDocumentos() {
    f = document.lancamento;
    f.mod.value = 'est';
    f.form.value = 'nota_fiscal';
    f.submenu.value = 'consultarDocumentos';
    f.submit();
} // submitConsultaDocumentos

function submitGerarEspelho() {
    f = document.lancamento;
    f.mod.value = 'est';
    f.form.value = 'nota_fiscal';
    f.submenu.value = 'gerarEspelho';
    f.submit();
} // submitConsultaDocumentos

function submitConsultaRecibo($id) {
...
function receivesResponse(response){

    debugger
    Swal.close(); // Fecha o SweetAlert de loading
     
    switch (response.status) {
        case "success": // Nota emitida

            var botaoVoltar = document.getElementById("btnVoltar");
            if (botaoVoltar) {
                botaoVoltar.click();
            }
            printDanfe(response.idNf);

            break;
        case "warning":

            //console.log("%c"+ response.errors.eReason, "background: blue; color: white; padding: 4px;");
