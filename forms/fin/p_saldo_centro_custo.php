...
require_once($dir . "/../../class/fin/c_saldo_centro_custo.php");

//Class p_saldo_centro_custo
Class p_saldo_centro_custo extends c_saldo_centro_custo {
...
function __construct() {
    ...
    $this->smarty->assign('titulo', "Saldo do centro de custo");
  ...
  switch ($this->m_submenu){
            ...
            case 'alterar':
                    if ($this->verificaDireitoUsuario('FinSaldo', 'A')){
                            $saldo = $this->select_saldo_centro_custo();
                            $this->setCentroCusto($saldo[0]['CENTROCUSTO']);
            ...
            case 'inclui':
                    if ($this->verificaDireitoUsuario('FinSaldo', 'I')){
                            if ($this->existeSaldo()){
                                    ...
                                    $this->desenhaCadastroSaldo("SALDO JÁ EXISTENTE NESTA DATA, ALTERE O CENTRO DE CUSTO OU A DATA", "alerta");}
                            else {
...
function desenhaCadastroSaldo($mensagem=NULL, $tipoMsg=NULL){
    ...
    if ($this->m_submenu=='alterar'):
        ...
        $this->smarty->assign('centrocusto_id', $this->m_par[2]);
    endif;


    $this->smarty->display('saldo_cadastro_centro_custo.tpl');
...
function mostraSaldo($mensagem){
	...
	$contaPesq_names[0] = '';
  	for ($i=0; $i < count($result); $i++){
		$contaPesq_ids[$i+1] = $result[$i]['CENTROCUSTO'];
	...
	$this->smarty->display('saldo_mostra_centro_custo.tpl');
...
$saldo = new p_saldo_centro_custo();
