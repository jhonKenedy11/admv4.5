...
//Class P_FLUXO_CAIXA
Class p_consolidacao extends c_lancamento {
...
function mostraConsolidacao($mensagem){
    ...
    $fluxo = new c_saldo();
    $reg_saldoIni = $fluxo->saldoContaDiaAnterior($this->m_letra);
    if ($reg_saldoIni[0]['SALDO'] != ''){
            $saldoInicial = $reg_saldoIni[0]['SALDO'];
    }else {
            $saldoInicial = 0;	
    } 

    $reg_saldo = $fluxo->saldoContaPeriodo($this->m_letra);

    $reg_lancamento = new c_lancamento();
    $reg_lanc = $this->select_lancamento_letra($this->m_letra);
    ...
    $this->smarty->assign('graSaldo', $arrSaldo);
    
    $this->smarty->display('consolidacao_mostra.tpl');
...
$consolidacao = new p_consolidacao();


$consolidacao->controle();
