...
//Class P_FLUXO_CAIXA
Class p_rel_slip_imprime extends c_lancamento {
...
function mostraLancPed($mensagem){
    ...
    $arrLanc = explode("|", $this->m_letra);
    
    if ($this->getId() == ''){
        for ($i=0; $i < count($arrLanc); $i++){
            if ($arrLanc[$i] != ''){
                $this->setId($arrLanc[$i]);
                $result = $this->select_lancamento();
                $lanc[$i] = $result[0];
            }
        }
    }else{
    ...
    $this->smarty->display('rel_slip_imprime.tpl');
...
$lancPed = new p_rel_slip_imprime();
