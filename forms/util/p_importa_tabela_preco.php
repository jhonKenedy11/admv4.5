...
include_once($dir . "/../../bib/SimpleXLSX.php");
...
//Class P_situacao
class p_importa_tabela_preco extends c_conta
{
    ...
    public function importarXlxs() {
        try {
            $colunas = null;
            $linhas = null;
            
            // Valida colunas básicas
            $codColuna = $this->m_par[2];
            $desColuna = $this->m_par[3];
            $precoColuna = $this->m_par[4];
            $ipiColuna = $this->m_par[5];
            $ncmColuna = $this->m_par[6];
            $marcaColuna = $this->m_par[7];
    
            // Tenta carregar o arquivo XLSX
            $planilhas = new SimpleXLSX($this->m_tmp);
            
            // Valida se o arquivo foi aberto corretamente
            if (!$planilhas || !$planilhas->success()) {
                throw new Exception("Arquivo inválido ou corrompido.");
            }
    
            list($colunas, $linhas) = $planilhas->dimension();
            
            // Valida se as colunas necessárias existem
            $requiredColumns = [$codColuna, $desColuna, $precoColuna, $ipiColuna, $ncmColuna, $marcaColuna];
            foreach ($requiredColumns as $col) {
                if ($col >= $colunas) {
                    throw new Exception("O arquivo está faltando colunas obrigatórias.");
                }
            }
            ...
            foreach ($planilhas->rows() as $chave => $valor) {
                if ($chave >= 1) {

                    $codOriginal  = trim($valor[$codColuna]);
                    $descricao    = trim($valor[$desColuna]);
                    $preco        = trim($valor[$precoColuna]);
                    $ipi          = trim($valor[$ipiColuna]);
                    $ncm        = trim($valor[$ncmColuna]);
                    $marca      = trim($valor[$marcaColuna]);

                    $codOriginal  = ($codOriginal == "" || $codOriginal == NULL) ? 'NULL' : $codOriginal;
                    $descricao  = ($descricao == "" || $descricao == NULL) ? 'NULL' : $descricao;
                    $preco  = ($preco == "" || $preco == NULL) ? 0 : $preco;
                    $ipi  = ($ipi == "" || $ipi == NULL) ? 0 : $ipi;
                    $ncm        = ($ncm == "" || $ncm == NULL) ? 'NULL' : $ncm;
                    $marca  = ($marca == "" || $marca == NULL) ? 'NULL' : $marca;
                    $precoVenda = ($preco * $this->m_par[1]);

                    $sql = "INSERT INTO EST_TABELA_FORNECEDOR (fornecedor, codoriginal, descricao, preco, ipi,  ncm, marca, precovenda) VALUES ";
                    $sql .= "(" . $this->m_par[0] . ", '" . $codOriginal . "', '" . $descricao . "', " . $preco . ", '" . $ipi . "', " . $ncm . ", " . $marca . "," . $precoVenda . ")";
            ...
            // Mensagem de sucesso APÓS commit
...
// ExcelFile($filename, $encoding);
    ...
    */
    ...
    function mostraImporta($mensagem)
    {
        ...
        $arqImporta_ids[0] = "fornecedor";
        $arqImporta_names[0] = "Fornecedor";
        ...
        $this->smarty->assign('codTabela', 1);
        $this->smarty->assign('descTabela', 2);
        $this->smarty->assign('precoTabela', 3);
        $this->smarty->assign('ipiTabela', 4);
        $this->smarty->assign('ncmTabela', 5);
        $this->smarty->assign('marcaTabela', 6);

        $this->smarty->display('importa_tabela_preco_mostra.tpl');
    }
...
$importa = new p_importa_tabela_preco();
