...
class p_conta extends c_conta
{
    ...
    function __construct($submenu, $letra, $opcao)
    {
        ...
        $this->m_param = (isset($parmGet['param']) ? $parmGet['param'] : (isset($parmPost['param']) ? $parmPost['param'] : ''));


        //obras
        $this->setNumArt(isset($parmGet['num_art']) ? $parmGet['num_art'] : (isset($parmPost['num_art']) ? $parmPost['num_art'] : ''));
        $this->setDescProjeto(isset($parmGet['desc_projeto']) ? $parmGet['desc_projeto'] : (isset($parmPost['desc_projeto']) ? $parmPost['desc_projeto'] : ''));
        $this->setNumCno(isset($parmGet['num_cno']) ? $parmGet['num_cno'] : (isset($parmPost['num_cno']) ? $parmPost['num_cno'] : ''));
        $this->setNumCrea(isset($parmGet['num_crea']) ? $parmGet['num_crea'] : (isset($parmPost['num_crea']) ? $parmPost['num_crea'] : ''));
        $this->setResponsavelTecnico(isset($parmGet['responsavel_tecnico']) ? $parmGet['responsavel_tecnico'] : (isset($parmPost['responsavel_tecnico']) ? $parmPost['responsavel_tecnico'] : ''));
        $this->setIdObra(isset($parmGet['id_obra']) ? $parmGet['id_obra'] : (isset($parmPost['id_obra']) ? $parmPost['id_obra'] : ''));
        $this->setIdCliente(isset($parmGet['id_cliente']) ? $parmGet['id_cliente'] : (isset($parmPost['id_cliente']) ? $parmPost['id_cliente'] : ''));
        $this->setStatusObra(isset($parmGet['status_obra']) ? $parmGet['status_obra'] : (isset($parmPost['status_obra']) ? $parmPost['status_obra'] : 'A'));
        $this->idAnexo = (isset($parmGet['idAnexo']) ? $parmGet['idAnexo'] : (isset($parmPost['idAnexo']) ? $parmPost['idAnexo'] : null));

        //ajax
        $this->ajax_pedidoId = (isset($parmPost['pedidoId']) ? $parmPost['pedidoId'] : "");
        ...
        if ($this->m_opcao == 'os') {
            ...
            $this->smarty->assign('disableSort', "[ 4, 6 ]");
        }
        ...
        switch ($this->m_submenu) {
            ...
            case 'alterar':
                if ($this->verificaDireitoUsuario('FinPessoa', 'C')) {
                    if ($this->m_param !== null and $this->m_param !== '') {
                        $this->setId($this->m_param);
                    }
                    $this->busca_conta();
            ...
            case 'buscaPedidosCliente':
                ...
                die;
                //endereco entrega inicio
            case 'insertAddress':

                try {
                    $this->setId($_POST['idCliente'] ?? null);
                    $this->setIdAddress($_POST['id_address'] ?? null);
                    $this->setTitulo($_POST['tituloEndereco'] ?? '');
                    $this->setObs($_POST['descricao'] ?? '');
                    $this->setFoneArea($_POST['ddd'] ?? '');
                    $this->setFone($_POST['fone'] ?? '');
                    $this->setContato($_POST['foneContato'] ?? '');
                    ...
                    if (empty($this->getIdAddress())) {
                        if (empty($_POST['idCliente'])) {
                            throw new Exception('ID do cliente não fornecido');
                        }
                        $result = $this->insert_address();
                    } else {
                        $result = $this->alter_address();
                    }
                ...
                break;
            case 'combo_busca_endereco':

                $consulta = new c_banco();
                $sql = "SELECT ID, TITULOEND FROM FIN_CLIENTE_ENDERECO ";
                $sql .= "WHERE CLIENTE = '" . $this->id_searchAddress . "' AND STATUS = 'A';";
            ...
            case 'cadastraObra':
                if ($this->verificaDireitoUsuario('FinPessoa', 'I')) {
                    if (!empty($this->id_obra)) {
                        $success = $this->alteraObra();
                    } else if (empty($this->id_obra)) {
                        $success = $this->incluiObra();
                    }
            ...
            case 'listaObrasAjax':
                if ($this->verificaDireitoUsuario('FinPessoa', 'C')) {
                    $obras = $this->selectObras($this->getId());
                    if ($obras) {
                        header('Content-type: application/json');
            ...
            case 'salvarAnexoObra':
                try {
                    ...
                    $resultado = $this->salvarAnexoObra($dados);
                    echo json_encode($resultado);
                } catch (Exception $e) {
                    echo json_encode([
                        'success' => false,
                        'message' => 'Erro ao processar anexo: ' . $e->getMessage()
                    ]);
                }
                break;

            case 'carregarAnexosObra':
                try {
                    $id_obra = (int)$_GET['id_obra'];
                    $resultado = $this->selectAnexoObra($id_obra);
            ...
            case 'excluiAnexoObra':
                try {
                    $id_anexo = (int)$_GET['id_anexo'];
                    $resultado = $this->excluiAnexoObra($id_anexo);

                    if ($resultado) {
                        echo json_encode(['success' => true]);
                    } else {
                        echo json_encode(['success' => false, 'message' => 'Falha ao excluir anexo']);
                    }
                } catch (Exception $e) {
                    echo json_encode([
                        'success' => false,
    ...
    function desenhaCadastroConta($mensagem = NULL, $tipoMsg = NULL)
    {
        ...
        //pedidos cadastrados
        $consultaPed = new c_pedidoVenda();
        $lancPedidos = $consultaPed->selectVerificaPedido($this->getId());
        $this->smarty->assign('lancPedidos', $lancPedidos);


        //search customer orders
        if (($this->getId() !== '') and ($this->getId() !== null)) {
            $deliveryAddress = $this->search_customer_order($this->getId());
        }
        ...
        //responsavel tecnico
        $consulta = new c_banco();
        $sql = "select usuario as id, NOMEREDUZIDO from amb_usuario where tipo = 'R'";
        $consulta->exec_sql($sql);
        $consulta->close_connection();
        ...
        foreach ($result as $row) {
            $responsavel_tecnico_ids[] = $row['ID'];
            $responsavel_tecnico_names[] = $row['NOMEREDUZIDO'];
        }
        $responsaveis = array_combine($responsavel_tecnico_ids, $responsavel_tecnico_names);
        $this->smarty->assign('responsavel_tecnico_ids', $responsavel_tecnico_ids);
        $this->smarty->assign('responsavel_tecnico_names', $responsavel_tecnico_names);
        $this->smarty->assign('responsavel_tecnico_id', $this->getResponsavelTecnico());
        $this->smarty->assign('responsaveis', $responsaveis);
...
$conta = new p_conta(
    $_POST['submenu'] == '' ? $_REQUEST['submenu'] : $_POST['submenu'],
    $_POST['letra'],
    $_REQUEST['opcao']
);
...
if (isset($_POST['filial'])) {
    $conta->setCentroCusto($_POST['filial']);
} else {
    $conta->setCentroCusto('');
};
if (isset($_REQUEST['id'])) {
    $conta->setId($_REQUEST['id']);
} else {
    $conta->setId('');
};
if (isset($_POST['nome'])) {
...
if (isset($_POST['nomeReduzido'])) {
    $conta->setNomeReduzido($_POST['nomeReduzido']);
} else {
    $conta->setNomeReduzido('');
};
if (isset($_POST['pessoa'])) {
    $conta->setPessoa($_POST['pessoa']);
} else {
    $conta->setPessoa('');
};
if (isset($_POST['cnpjCpf'])) {
    $conta->setCnpjCpf($_POST['cnpjCpf']);
} else {
    $conta->setCnpjCpf('');
};
if (isset($_POST['dataNascimento'])) {
    $conta->setDataNascimento($_POST['dataNascimento']);
} else {
    $conta->setDataNascimento(NULL);
};
if (isset($_POST['ieRg'])) {
    $conta->setIeRg($_POST['ieRg']);
} else {
    $conta->setIeRg('');
};
if (isset($_POST['im'])) {
    $conta->setIm($_POST['im']);
} else {
    $conta->setIm('');
};
if (isset($_POST['cep'])) {
...
if (isset($_POST['tipo'])) {
    $conta->setTipo($_POST['tipo']);
} else {
    $conta->setTipo('');
};
if (isset($_POST['titulo'])) {
    $conta->setTitulo($_POST['titulo']);
} else {
    $conta->setTitulo('');
};
if (isset($_POST['endereco'])) {
    $conta->setEndereco($_POST['endereco']);
} else {
    $conta->setEndereco('');
};
...
if (isset($_POST['complemento'])) {
    $conta->setComplemento($_POST['complemento']);
} else {
    $conta->setComplemento('');
};
if (isset($_POST['bairro'])) {
    $conta->setBairro($_POST['bairro']);
} else {
    $conta->setBairro('');
};
if (isset($_POST['cidade'])) {
    $conta->setCidade($_POST['cidade']);
} else {
    $conta->setCidade('');
};
if (isset($_POST['codMunicipio'])) {
    $conta->setCodMunicipio($_POST['codMunicipio']);
} else {
    $conta->setCodMunicipio('');
};
if (isset($_POST['suframa'])) {
    $conta->setSuframa($_POST['suframa']);
} else {
    $conta->setSuframa('');
};
if (isset($_POST['estado'])) {
    $conta->setEstado($_POST['estado']);
} else {
    $conta->setEstado('');
};
if (isset($_POST['fone'])) {
    $conta->setFone($_POST['fone']);
} else {
    $conta->setFone('');
};
...
if (isset($_POST['faxArea'])) {
    $conta->setFaxArea($_POST['faxArea']);
} else {
    $conta->setFaxArea('');
};
if (isset($_POST['faxNum'])) {
    $conta->setFaxNum($_POST['faxNum']);
} else {
    $conta->setFaxNum('');
};
if (isset($_POST['contato'])) {
    $conta->setContato($_POST['contato']);
} else {
    $conta->setContato('');
};
...
if (isset($_POST['emailNfe'])) {
    $conta->setEmailNfe($_POST['emailNfe']);
} else {
    $conta->setEmailNfe('');
};
if (isset($_POST['homePage'])) {
    $conta->setHomePage($_POST['homePage']);
} else {
    $conta->setHomePage('');
};
if (isset($_POST['classe'])) {
    $conta->setClasse($_POST['classe']);
} else {
    $conta->setClasse('');
};
if (isset($_POST['atividade'])) {
    $conta->setAtividade($_POST['atividade']);
} else {
    $conta->setAtividade('');
};
if (isset($_POST['vendedor'])) {
    $conta->setRepresentante($_POST['vendedor']);
} else {
    $conta->setRepresentante('');
};
if (isset($_POST['userLogin'])) {
    $conta->setUserLogin($_POST['userLogin']);
} else {
    $conta->setUserLogin('');
};
if (isset($_POST['senhaLogin'])) {
    $conta->setSenhaLogin($_POST['senhaLogin']);
} else {
    $conta->setSenhaLogin('');
};
if (isset($_POST['limiteCredito'])) {
    $conta->setLimiteCredito($_POST['limiteCredito']);
} else {
    $conta->setLimiteCredito('');
};
if (isset($_POST['obs'])) {
    $conta->setObs($_POST['obs']);
} else {
    $conta->setObs('');
};
if (isset($_POST['transversal1'])) {
    $conta->setTransversal1($_POST['transversal1']);
} else {
    $conta->setTransversal1('');
};
if (isset($_POST['transversal2'])) {
    $conta->setTransversal2($_POST['transversal2']);
} else {
    $conta->setTransversal2('');
};
if (isset($_POST['referencia'])) {
    $conta->setReferencia($_POST['referencia']);
} else {
    $conta->setReferencia('');
};
if (isset($_POST['referencia'])) {
...
if (isset($_POST['regimeEspecialST'])) {
    $conta->setRegimeEspecialST($_POST['regimeEspecialST']);
} else {
    $conta->setRegimeEspecialST('N');
};
if (isset($_POST['regimeEspecialSTMsg'])) {
    $conta->setRegimeEspecialSTMsg($_POST['regimeEspecialSTMsg']);
} else {
    $conta->setRegimeEspecialSTMsg('N');
};
if (isset($_POST['regimeEspecialSTMT'])) {
    $conta->setRegimeEspecialSTMT($_POST['regimeEspecialSTMT']);
} else {
    $conta->setRegimeEspecialSTMT('N');
};
if (isset($_POST['contribuinteICMS'])) {
    $conta->setContribuinteICMS($_POST['contribuinteICMS']);
} else {
    $conta->setContribuinteICMS('N');
};
if (isset($_POST['consumidorFinal'])) {
    $conta->setConsumidorFinal($_POST['consumidorFinal']);
} else {
