...
Class p_os_imprime extends c_atendimento {
...
//---------------------------------------------------------------
    function mostraPedidoImprime($mensagem=NULL, $tipoMsg=NULL) {        
        ...
        $html = $this->smarty->fetch('os_imprime_new.tpl');  
        $filePath =  ADMraizCliente."/images/doc/os".$this->getId(); 
                 
        $filename = ADMraizCliente."/images/doc/os".$this->getId().".pdf";
        $options = new Options();
 ...
 //
        //chmod($filename, 0777);
        $dompdf->stream('os.pdf', array("Attachment"=>0));
        

        
        //$this->smarty->display('os_imprime.tpl');
    }
