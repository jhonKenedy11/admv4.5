...
Class p_rel_compra_encomenda extends c_estoque_rel {
...
//---------------------------------------------------------------
//---------------------------------------------------------------
function relatorioCompraEncomenda($mensagem){
    ...
    for($i=0;$i<count($lanc);$i++){
        ...
        $lanc[$i]['DISPONIVEL'] = $produtoQuant[0]['DISPONIVEL'];
        if($lanc[$i]['DISPONIVEL'] === null){
            $lanc[$i]['DISPONIVEL'] = 0;
        }

        if($lanc[$i]['ENCOMENDA'] > 0){
            
            $lanc[$i]['COMPRA'] = $lanc[$i]['ENCOMENDA'] - $lanc[$i]['ESTOQUE'];

            $resultProduto[] = $lanc[$i];
        }
        $p++;
        
    }

    for($i = 0; $i < count($resultProduto); $i++){
        $resultProduto[$i]['NRITEM'] = $i+1;
    }


    $this->smarty->assign('pedido', $resultProduto);
    
    $this->smarty->display('relatorio_compra_encomenda.tpl');
}
...
$produto = new p_rel_compra_encomenda();
