...
function mostraDashboard($mensagem=NULL){
    ...
    */
    ...
    $sql .="group by USRFATURA, METAS, VENDEDOR;";
    ...
    $sql = "select round(Sum(n.TOTALNF),2) as TOTALNF, round(Sum(n.LUCROBRUTO),2) as LUCROBRUTO, ";
    $sql .= "round(SUM(n.CUSTOTOTAL),2) as CUSTOTOTAL, round(Sum(m.META),2) as TOTALMETA, ";
    $sql .= "((round(Sum(m.META),2) - round(Sum(n.TOTALNF),2) ) / (SELECT TIMESTAMPDIFF(DAY,'".$ano."-".$mes."-01','".$ano."-".$mes."-".$ultimo_dia."'))) as METADIARIA,  ";
    $sql .= "(round(Sum(m.META),2) - round(Sum(n.TOTALNF),2)) as METAASERALCANCADA ";
    ...
    if (isset($totais[0]['TOTALNF']) > 0) {
        $pontoequilibrio = round((($totais[0]['LUCROBRUTO'] - $despesas[0]['TOTAL']) / $totais[0]['TOTALNF']), 2);
    } else {
        $pontoequilibrio = 0;
    }
    
    $this->smarty->assign('pontoequilibrio', $pontoequilibrio);
