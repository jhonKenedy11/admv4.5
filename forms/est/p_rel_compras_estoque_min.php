...
/**
 * @package   astec
 * @name      p_rel_compras_estoque_mins
 * @version   3.0.00
 * @copyright 2016
...
//Class p_rel_compras_estoque_min
Class p_rel_compras_estoque_min extends c_estoque_rel {
...
//---------------------------------------------------------------
    function controle() {
        switch ($this->m_submenu) {
            default:
                $this->relatorioComprasEstoqueMin('');
        } //switch
    }
...
//---------------------------------------------------------------
//---------------------------------------------------------------
function relatorioComprasEstoqueMin($mensagem){
    ...
    $lanc = $this->select_relatorio_compras() ?? [];
    ...
    for($i=0;$i<count($lanc);$i++){
        ...
        if($produtoQuant[0]['QUANTIDADE'] <= 0 AND 
            $produtoQuant[0]['QUANTIDADE'] < $lanc[$i]['QUANTMINIMA']){
    ...
    $this->smarty->display('relatorio_compras_estoque_min.tpl');
}
...
$produto = new p_rel_compras_estoque_min();
