...
class p_nfe_40 extends c_user
{
  ...
  */
    ...
    private function processDistDfeDocZip($doc, array &$failNf, int &$incrementa, array &$state)
    {
        ...
        $xmlRaw = gzdecode(base64_decode($doc->nodeValue));
        ...
        if ($tipo == 'resNFe') {
            ...
            if (!c_nota_fiscal::existNotaNumClient($numeroNF, $serie, $cliente)) {
                ...
                $idNf = $this->distNfeGravaNota($objetoNotaFiscal, (string) $xml->chNFe, $failNf, $incrementa);
                if ($idNf) {
                    (new c_manifesto_fiscal_sefaz())->enviaEventoManifesto($idNf, 'confirma');
                }
            }
        } elseif ($tipo == 'procNF') {
