...
class p_nfe_50 extends c_user{
     ...
     * https://github.com/nfephp-org/nfephp/blob/master/exemplos/NFe/4.00testaMakeNFe.php
     */
    public function gera_XML($idNf) {
        $dir = (__DIR__);
        ...
        ** Node referente a Nota Fiscal referenciada modelo 1 ou 2 -- Verificar **
        ** Node referente a Nota Fiscal referenciada de produtor rural  -- Verificar **
        ** Node referente aos CTe referenciados  -- Verificar **
        ** Node referente aos ECF referenciados  -- Verificar **
        ...
        *** */        
        if ($nfArray[0]['MODELO'] == 55):
            ...
            *** */        
            $xLgr = $this->removeAcentos($pessoaDestArray[0]['ENDERECO']);
            ...
            switch ($crt):
                case '1': //ICMSSN - Tributação ICMS pelo Simples Nacional - CRT (Código de Regime Tributário) = 1 
                    ...
                    switch ($produtoArray[$i]['TRIBICMS']){
                        case '101': //101=Tributada pelo Simples Nacional com permissão de crédito. (v2.0)
                            $orig = $produtoArray[$i]['ORIGEM'];
                            $csosn = '101'; 
                        ...
                        case '400': 
                            ...
                            $csosn = '400';  
                            break;
                        case '201': //201=Tributada pelo Simples Nacional com permissão de crédito e com cobrança do ICMS por Substituição Tributária (v2.0)                             $orig = $produtoArray[$i]['ORIGEM'];
                            $csosn = '201'; 
                            $pCredSN = $produtoArray[$i]['PCREDSN'];//Alíquota aplicável de cálculo do crédito (SIMPLES NACIONAL). 
                            $vCredICMSSN = $produtoArray[$i]['VCREDICMSSN']; //Valor crédito do ICMS que pode ser aproveitado nos termos do art. 23 da LC 123 (SIMPLES NACIONAL)
                            break;                        
                        case '202': //202=Tributada pelo Simples Nacional sem permissão de crédito e com cobrança do ICMS por Substituição Tributária;
                            $orig = $produtoArray[$i]['ORIGEM'];
                            $csosn = '202'; 
                            ...
                            break;
                        case '203': //203- Isenção do ICMS nos Simples Nacional para faixa de receita bruta e com cobrança do ICMS por Substituição Tributária (v2.0)
                            $orig = $produtoArray[$i]['ORIGEM'];
                            $csosn = '203'; 
                            $modBC = $produtoArray[$i]['MODBC'];
                            $pMVAST = $produtoArray[$i]['PERCMVAST']; //Percentual da margem de valor Adicionado do ICMS ST
                            ...
                            $vICMSST = $produtoArray[$i]['VALORICMSST']; //Valor do ICMS ST
                            break;
                        case '500': //500=ICMS cobrado anteriormente por substituição tributária(substituído) ou por antecipação. (v2.0)
                            $orig = $produtoArray[$i]['ORIGEM'];
                            $csosn = '500';
                    ...
                    break;   
                case '2': //ICMSSN - Tributação ICMS pelo Simples Nacional - excesso sublimite de receita bruta.
            ...
            if ($crt == '1') {
                ...
                if ($produtoArray[$i]['CSTIPI'] == '') {
                    if ((($nfArray[0]['FINALIDADEEMISSAO'] == 10) and ($crt == '1')) or 
                        (($nfArray[0]['FINALIDADEEMISSAO'] == 4) and ($produtoArray[$i]['ALIQIPI']) > 0)){
                        $CST = '99';
                    } else {
                        $CST = '53';
                    }   
...
*/        
        ...
        */
        ...
        >
        > tPag é usualmente:
         * - 01 = Dinheiro - 0x = Cheque- 03 = Cartão de Crédito - 04 = Cartão de Débito - 05 = Credito Loja - 10 = Vale Alimentação
        > - 11 = Vale Refeição - 12 = Vale Presente - 13 = Vale Combustivel - 15 = Boleto Bancário - 90 = Sem pagamento - 99 = Outros 
        >
...
*/            
        ...
        try {
            ...
            // https://github.com/nfephp-org/sped-nfe/blob/master/docs/StartGuide.md
            // Esclarecimentos sobre TIMEOUT
            // https://github.com/nfephp-org/sped-nfe/blob/master/docs/TimeOut.md
        ...
        try {
            ...
            //file_put_contents(BASE_DIR_ENTRADA , $xmlProtocolado);
        ...
        if (($aResposta['cStatus'] == '100') or ($aResposta['cStatus'] == '105')):
            ...
            $nfOBJ->setChNFe($status->protNFe->infProt->chNFe);
            $nfOBJ->setDhRecbto($status->dhRecbto);
            $nfOBJ->setNProt($status->protNFe->infProt->nProt);
            $nfOBJ->setDigVal($status->protNFe->infProt->digVal);
            $nfOBJ->setVerAplic($status->verAplic);
