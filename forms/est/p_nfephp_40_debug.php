    ...
    public function gera_XML($idNf, $filial, $tipoNf, $conn = null, $gerarXML = null)
    {
        ...
        */
        if ($modFrete != 9) {
            ...
            // ===== DEBUG TAG TRANSPORTA =====
            if ($modFrete == '1') {
                echo "<h3>🏷️ DEBUG TAG TRANSPORTA (ANTES DE CRIAR):</h3>";
                echo "<pre>";
                echo "modFrete: " . $modFrete . "\n";
                echo "std->xNome: " . $std->xNome . "\n";
                echo "std->CNPJ: " . $std->CNPJ . "\n";
                echo "std->CPF: " . $std->CPF . "\n";
                echo "std->IE: " . $std->IE . "\n";
                echo "std->xEnder: " . $std->xEnder . "\n";
                echo "std->xMun: " . $std->xMun . "\n";
                echo "std->UF: " . $std->UF . "\n";
                echo "</pre>";
            }
            // ===== FIM DEBUG TAG TRANSPORTA =====
        ...
        */
        ...
        if ($this->m_empresaid == 3) {
            ...
            // ===== DEBUG TRANSPORTADOR FOB =====
            // Verificar se é FOB (modFrete = 1) e exibir dados do transportador
            if ($modFrete == '1') {
                echo "<h2>🔍 DEBUG TRANSPORTADOR FOB - NOTA: " . $idNf . "</h2>";
                echo "<h3>📋 DADOS DA NOTA FISCAL:</h3>";
                echo "<pre>";
                echo "ID Nota: " . $idNf . "\n";
                echo "ModFrete: " . $modFrete . " (FOB - Por conta do destinatário)\n";
                echo "Transportador ID: " . $nfArray[0]['TRANSPORTADOR'] . "\n";
                echo "Cliente: " . $nfArray[0]['PESSOA'] . "\n";
                echo "</pre>";
                
                echo "<h3>🚛 DADOS DO TRANSPORTADOR:</h3>";
                if (is_array($transpArray) && !empty($transpArray)) {
                    echo "<pre>";
                    echo "Nome: " . $transpArray[0]['NOME'] . "\n";
                    echo "CNPJ/CPF: " . $transpArray[0]['CNPJCPF'] . "\n";
                    echo "Pessoa: " . $transpArray[0]['PESSOA'] . "\n";
                    echo "Inscrição Estadual: " . $transpArray[0]['INSCESTRG'] . "\n";
                    echo "Endereço: " . $transpArray[0]['ENDERECO'] . ", " . $transpArray[0]['NUMERO'] . "\n";
                    echo "Complemento: " . $transpArray[0]['COMPLEMENTO'] . "\n";
                    echo "Bairro: " . $transpArray[0]['BAIRRO'] . "\n";
                    echo "Cidade: " . $transpArray[0]['CIDADE'] . "\n";
                    echo "UF: " . $transpArray[0]['UF'] . "\n";
                    echo "</pre>";
                } else {
                    echo "<p style='color: red;'>❌ TRANSPORTADOR NÃO ENCONTRADO!</p>";
                }
                
                echo "<h3>👤 DADOS DO DESTINATÁRIO (FALLBACK):</h3>";
                if (is_array($pessoaDestArray) && !empty($pessoaDestArray)) {
                    echo "<pre>";
                    echo "Nome: " . $pessoaDestArray[0]['NOME'] . "\n";
                    echo "CNPJ/CPF: " . $pessoaDestArray[0]['CNPJCPF'] . "\n";
                    echo "Pessoa: " . $pessoaDestArray[0]['PESSOA'] . "\n";
                    echo "Inscrição Estadual: " . $pessoaDestArray[0]['INSCESTRG'] . "\n";
                    echo "Endereço: " . $pessoaDestArray[0]['ENDERECO'] . ", " . $pessoaDestArray[0]['NUMERO'] . "\n";
                    echo "Complemento: " . $pessoaDestArray[0]['COMPLEMENTO'] . "\n";
                    echo "Bairro: " . $pessoaDestArray[0]['BAIRRO'] . "\n";
                    echo "Cidade: " . $pessoaDestArray[0]['CIDADE'] . "\n";
                    echo "UF: " . $pessoaDestArray[0]['UF'] . "\n";
                    echo "</pre>";
                }
                
                echo "<h3>🏷️ DADOS QUE SERÃO ENVIADOS PARA A TAG TRANSPORTA:</h3>";
                echo "<pre>";
                echo "xNome: " . (isset($xNome) ? $xNome : 'NÃO DEFINIDO') . "\n";
                echo "CNPJ: " . (isset($CNPJ) ? $CNPJ : 'NÃO DEFINIDO') . "\n";
                echo "CPF: " . (isset($CPF) ? $CPF : 'NÃO DEFINIDO') . "\n";
                echo "IE: " . (isset($IE) ? $IE : 'NÃO DEFINIDO') . "\n";
                echo "xEnder: " . (isset($xEnder) ? $xEnder : 'NÃO DEFINIDO') . "\n";
                echo "xMun: " . (isset($xMun) ? $xMun : 'NÃO DEFINIDO') . "\n";
                echo "UF: " . (isset($UF) ? $UF : 'NÃO DEFINIDO') . "\n";
                echo "</pre>";
                
                echo "<h3>📦 DADOS DE VOLUME:</h3>";
                echo "<pre>";
                echo "Volume: " . $nfArray[0]["VOLUME"] . "\n";
                echo "Espécie: " . $nfArray[0]["VOLESPECIE"] . "\n";
                echo "Marca: " . $nfArray[0]["VOLMARCA"] . "\n";
                echo "Peso Líquido: " . $nfArray[0]["VOLPESOLIQ"] . "\n";
                echo "Peso Bruto: " . $nfArray[0]["VOLPESOBRUTO"] . "\n";
                echo "Placa Veículo: " . $nfArray[0]["PLACAVEICULO"] . "\n";
                echo "Código ANTT: " . $nfArray[0]["CODANTT"] . "\n";
                echo "</pre>";
                
                echo "<div style='background: #ffeb3b; padding: 20px; margin: 20px 0; border: 2px solid #ff9800; border-radius: 5px;'>";
                echo "<h2>⚠️ PARADA DE DEBUG ATIVADA</h2>";
                echo "<p><strong>O envio da NFe foi interrompido para debug do transportador FOB.</strong></p>";
                echo "<p>Para continuar o envio, remova ou comente este bloco de debug.</p>";
                echo "<p>Nota ID: <strong>" . $idNf . "</strong></p>";
                echo "</div>";
                
                // PARAR A EXECUÇÃO AQUI PARA DEBUG
                die("🛑 EXECUÇÃO INTERROMPIDA PARA DEBUG - TRANSPORTADOR FOB");
            }
            // ===== FIM DEBUG TRANSPORTADOR FOB =====
