...
class p_nfe_40 extends c_user
{
  ...
  */
    ...
    public function consultaDistNfeIniciar($ultimaNsu)
    {
        ...
        if ($ultimaNsu == null || $ultimaNsu == '') {
            $novaNsu = '000000000000000';
        } else {
            $novaNsu = str_pad(strval(intval($ultimaNsu) + 1), strlen($ultimaNsu), "0", STR_PAD_LEFT);
        }
    ...
    {
        $conta = c_conta::existeContaCnpjCpf($cnpj);
    ...
    {
        if (!is_int($nf->incluiNotaFiscalManisfesto())) {
            $failNf[$incrementa++] = $chave;
        }
    }

    /** Processa um documento docZip retornado pela DistDFe. */
    private function processDistDfeDocZip($doc, array &$failNf, int &$incrementa)
    {
        $schema = $doc->getAttribute('schema');
        $xml = new SimpleXMLElement(gzdecode(base64_decode($doc->nodeValue)));
        ...
        if ($tipo == 'resNFe') {
            ...
            $cliente = $this->distNfeClienteId((string) $xml->CNPJ, $xml, $tipo);
            if (!c_nota_fiscal::existNotaNumClient($numeroNF, $serie, $cliente)) {
                ...
                $this->distNfeGravaNota($objetoNotaFiscal, (string) $xml->chNFe, $failNf, $incrementa);
            }
        } elseif ($tipo == 'procNF') {
            ...
            if (!c_nota_fiscal::existNotaNumClient($numero, $serie, $cliente)) {
                ...
                $this->distNfeGravaNota($objetoNotaFiscal, (string) $xml->protNFe->infProt->chNFe, $failNf, $incrementa);
            }
        }
    }
    ...
    public function consultaDistNfeUmLote()
    {
        ...
        $state['cStat'] = $cStat;
        $this->recordHeader($verAplic, $cStat, $xMotivo, $dhResp, $ultNSU, $maxNSU);
        ...
        if (in_array($cStat, ['137', '656', '589'], true)) {
            $this->distNfeLimpaSessao();
            return $this->distNfeJson($state, $cStat, self::distNfeMsgSefaz($cStat), true);
        }
        ...
        if (!empty($lote)) {
            ...
            foreach ($docs as $doc) {
                $this->processDistDfeDocZip($doc, $failNf, $incrementa);
                $docsProcessados++;
            }
        }
    ...
    public function gera_XML($idNf, $filial, $tipoNf, $conn = null, $gerarXML = null)
    {
        ...
        */
        ...
        for ($i = 0; $i < count($produtoArray); $i++) {
            ...
            $vlDifal = (float) ($produtoArray[$i]['VALORICMSUFDEST'] ?? 0);
            if ($bcDifal > 0 || $vlDifal > 0) {
                $p = $produtoArray[$i];
                $stdDifal = new stdClass();
                $stdDifal->item = $nItem;
                $stdDifal->vBCUFDest = number_format($bcDifal, 2, '.', '');
                $stdDifal->pICMSUFDest = number_format((float) ($p['ALIQICMSUFDEST'] ?? 0), 2, '.', '');
                $stdDifal->pICMSInter = number_format((float) ($p['ALIQICMSINTER'] ?? 0), 2, '.', '');
                $stdDifal->pICMSInterPart = number_format((float) (($p['ALIQICMSINTERPART'] ?? 0) ?: 100), 2, '.', '');
                $stdDifal->vICMSUFDest = number_format($vlDifal, 2, '.', '');
                $stdDifal->vICMSUFRemet = number_format((float) ($p['VALORICMSUFREMET'] ?? 0), 2, '.', '');
                if ((float) ($p['ALIQFCPUFDEST'] ?? 0) > 0.01) {
                    $stdDifal->vBCFCPUFDest = number_format((float) ($p['BCFCPUFDEST'] ?? $bcDifal), 2, '.', '');
                    $stdDifal->pFCPUFDest = number_format((float) $p['ALIQFCPUFDEST'], 2, '.', '');
                    $stdDifal->vFCPUFDest = number_format((float) ($p['VALORFCPUFDEST'] ?? 0), 2, '.', '');
                }
