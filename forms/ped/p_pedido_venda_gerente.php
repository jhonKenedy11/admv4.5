...
//Class P_situacao
Class p_pedido_venda_conferecia extends c_pedidoVenda {
...
//---------------------------------------------------------------
    function mostraPedidoGerente($mensagem) {
        // Se não houver filtro, define o padrão da semana atual
        if (empty($this->m_letra)) {
            $hoje = date('Y-m-d');
            $diaSemana = date('w');
            $primeiro = date('Y-m-d', strtotime("-{$diaSemana} days"));
            $ultimo = date('Y-m-d', strtotime("+".(6-$diaSemana)." days"));
            $this->m_letra = $primeiro . '|' . $ultimo . '||||||3|1|2|3';
        }
        if ($mensagem == 'A'){
...
$pedido = new p_pedido_venda_conferecia($_POST['submenu'], $_POST['letra'], $_POST['codProduto'], $_POST['qtdeConferido']);
