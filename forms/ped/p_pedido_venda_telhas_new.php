...
Class p_pedido_venda_telhas extends c_pedidoVenda {
    ...
    function controle() {
        switch ($this->m_submenu) {
            ...
            case 'atualizarCCEntrega':
                {
                    if ($this->m_par[0] > 0) {
                        $cceNovo = $this->m_par[0];
                        $this->setPedidoVenda();
                        $this->m_letra = "|||||||||".$this->getId();
                        $this->m_par = explode("|", $this->m_letra);                        
                        $cce = $this->getCentroCustoEntrega();
                        $situacao = $this->getSituacao();
                        $dataEntrega = $this->getDataEntrega('B');
                        if ((($dataEntrega == '') or ($dataEntrega == null)) and ($situacao <> 9)) {

                            if ($cceNovo == $cce){
                                $this->mostraPedido('Pedido '.$this->getId().'Alterado Centro Custo Entrega: '.$cce, 'alerta');
                            }else{
                                // VALIDA SE TEM ESTOQUE
                                $objPedidoTool = new c_pedidoVendaTools();
                                $msg = $objPedidoTool->validaPedido($this->getId(), $cceNovo);
                                
                                // ALTERA PARA O NOVO CCE
                                $this->atualizarField('centrocustoentrega',$cceNovo);
        
                                $situacao = $this->getSituacao();
                                if (!is_null($msg) ) {
                                    if (($situacao == 12) or ($situacao == 13) or ($situacao ==6)) { // 12 =APROVADO - 13 =ENCOMENDA
                                        $this->alteraPedidoEncomenda($this->getId()); // ENCOMENDA
                                        $this->mostraPedido('Pedido '.$this->getId().' - Alterado Centro Custo Entrega: '.$cceNovo.'<br> Enviado para ENCOMENDA: <br>'.$msg, 'alerta');
                                        break;
                                    }else{
                                        $this->desenhaCadastroPedido('Falta Produto Pedido: <br>'.$msg, 'alerta');
                                        // $this->mostraPedido('Pedido '.$this->getId().'Alterado Centro Custo Entrega: '.$cceNovo, 'alerta');
                                        break;
                                    }
                                }else{
                                    $this->mostraPedido('Pedido '.$this->getId().'Alterado Centro Custo Entrega: '.$cceNovo, 'alerta');
                        ...
                        } // data entrega situacao
                        else {
                            $this->desenhaCadastroPedido('Alerta-- Pedido não pode ser alterado Centro Custo Entrega!!!', 'alerta');
                        }
                    }
                }                
                break;
            ...
            case 'atulizarInfoItem': {
                    ...
                    if ($this->getCusto('B') > 0) {
                        ...
                        //$custo = str_replace('.', ',', $this->m_par[0]);
                        //$this->setCusto($custo);                                                    
                        // if ($this->m_par[5] == true ) {
                        //     c_produto::updateCustoCompra($this->m_par[4],$this->getCusto('B'));   
                        // }
                    ...
                    ($this->getSituacao() == 5))){ // COTAÇÃO
                        ...
                        if($est_aprovacao[0]['APROVACAO'] == 'M'){ // ****MARKUP MINIMO E APROVAÇÃO
                            ...
                            if ($pedMarkup <  $est_aprovacao[0]['MARKUPMIN']) { // MARCKUP ABAIXO APROVAÇÃO
                                if (($this->getSituacao()==12)   //******* 12- Aprovado
                                    //or (!($usrAprovacao == 0) and !($usrAprovacao == "") and !($usrAprovacao == "NULL")) 
                                ){
                                    ...
                                    if($this->m_dashboard_origem == 'dashboard_crm'){ // jhon
                                        $returnAjax = 'Pedido '.$this->getId().' em Aprovação!';
                                        echo $returnAjax;
                                    }else{
            ...
            case 'digita': //VOLTAR
                ...
                if($this->getId() != ''){ //salva dados do pedido
                    ...
                    $pedItem = $this->select_todos_pedidos_item($pedido);
                    if($pedItem != ''){
                        $sumTotal = 0;
                        for($i = 0; $i < count($pedItem); $i++ ){
                            $this->setId($pedItem[$i]['ID']);
                            $this->setNrItem($pedItem[$i]['NRITEM']);
                            $this->pedido_venda_item(true);
                            $qtde = $this->getQtSolicitada('F');
                            $qtde = str_replace('.', '',$qtde);
                            $qtde = str_replace(',', '.',$qtde);
                            $this->setQtSolicitada($qtde);
                            $this->alteraPedidoItemTelhas();
                            $sumTotal = $sumTotal + $pedItem[$i]['TOTAL'];
                        }
                    } 
                    
                    
                    $totalPed = $sumTotal + $this->getFrete('B') + 
                    $this->getDespAcessorias('B') - $this->getDesconto('B');
                    $this->setId($pedido);
                    if($this->getEmissao() == ''){
                        $this->setEmissao(date("Y-m-d"));
                    }
...
//---------------------------------------------------------------
    function mostraPedido($mensagem=NULL, $tipoMsg=NULL) {
        ...
        if (ADMSistema != 'PECAS') {
            $sql .= " AND ((TIPO = 0) or (TIPO = 5) or (TIPO = 6) or (TIPO = 7) or (TIPO = 9) or (TIPO = 10) or (TIPO = 11) or (TIPO = 12)  or (TIPO = 13))";
        }        
