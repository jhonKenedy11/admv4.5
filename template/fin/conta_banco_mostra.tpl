    ...
    <div class="">
      ...
      <div class="row">
        ...
        <div class="col-md-12 col-sm-12 col-xs-12">
          <div class="x_panel">
            <div class="x_title">
              <h2>Contas Banc&aacute;rias - Consulta
              </h2>
              {include file="../bib/msg.tpl"}
              <ul class="nav navbar-right panel_toolbox">
                <li><button type="button" class="btn btn-primary" onClick="javascript:submitCadastro('conta_banco');">
                ...
                <tbody>
                  ...
                  {section name=i loop=$lanc}
                    ...
                    <tr class="even pointer">
                      <td> {$lanc[i].CONTA} </td>
                      <td> {$lanc[i].NOMEINTERNO} </td>
                      <td> {$lanc[i].BANCO} </td>
                      <td> {$lanc[i].AGENCIA} </td>
                      <td> {$lanc[i].CONTACORRENTE} </td>
                      <td> {$lanc[i].CONTATO} </td>
                      <td> {$lanc[i].STATUS} </td>
                      <td class=" last">
                        <button type="button" class="btn btn-primary btn-xs"
                          onclick="javascript:submitAlterar('conta_banco','{$lanc[i].CONTA}');"><span
                            class="glyphicon glyphicon-pencil" aria-hidden="true"></span></button>
                        <button type="button" class="btn btn-danger btn-xs"
                          onclick="javascript:submitExcluir('conta_banco','{$lanc[i].CONTA}');"><span
