                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button type="submit" class="btn btn-primary btn-xs"
                                            onclick="javascript:submitAlterar('{$lanc[i].ID}', '{$lanc[i].SITUACAO}', '{$lanc[i].CLIENTE}');">
                                            <span class="glyphicon glyphicon-pencil" aria-hidden="true"
                                                data-toggle="tooltip" title="Editar"></span>
                                        </button>

                                        <button type="button" class="btn btn-danger btn-xs"
                                            {if $lanc[i].ID_SITUACAO == '6' or $lanc[i].ID_SITUACAO == '10'} disabled {/if}
                                            onclick="javascript:submitCancelar('{$lanc[i].ID}');">
                                            <span class="glyphicon glyphicon-remove" aria-hidden="true"
                                                data-toggle="tooltip" title="Cancelar"> </span>
                                        </button>

                                        <button type="button" class="btn btn-warning btn-xs"
                                            {if $lanc[i].ID_SITUACAO == '6' or $lanc[i].ID_SITUACAO == '10'} disabled {/if}
                                            onclick="javascript:submitCadastrarAtendimentoPedido('{$lanc[i].ID}');">
                                            <span class="glyphicon glyphicon-cog" aria-hidden="true" data-toggle="tooltip"
                                                title="Cadastro OS Pedido"></span>
                                        </button>
                                        ...
                                        <button type="button" class="btn btn-info btn-xs"
                                            onclick="javascript:imprimeOs('{$lanc[i].ID}');">
                                            <span class="glyphicon glyphicon-print" aria-hidden="true" data-toggle="tooltip"
                                                title="Imprimir"></span>
                                        </button>

                                        <button type="button" class="btn-group btn-group-toggle btn-xs"
                                            onclick="javascript:imprimeOsMecanico('{$lanc[i].ID}');">
                                            <span class="glyphicon glyphicon-print" aria-hidden="true" data-toggle="tooltip"
                                                title="Imprimir OS Mecanico"></span>
                                        </button>

                                        <button type="button" class="btn btn-success btn-xs"
                                            onclick="javascript:abrir('index.php?mod=cat&form=orcamento_imprime&opcao=imprimir&id={$lanc[i].ID}');">
                                            <span class="glyphicon glyphicon glyphicon-briefcase" aria-hidden="true"
                                                data-toggle="tooltip" title="Imprimir orçamento"></span>
                                        </button>
                                        <button type="button" class="btn btn-danger btn-xs"
                                            {if $lanc[i].ID_SITUACAO == '6' or $lanc[i].ID_SITUACAO == '10' or $lanc[i].ID_SITUACAO == '4'}
                                            disabled {/if} onclick="submitEstornarOs('{$lanc[i].ID}');">
                                            <span class="glyphicon glyphicon-refresh" aria-hidden="true"
                                                data-toggle="tooltip" title="Estornar ordem de serviço"></span>
                                        </button>
                                    </td>
                                </tr>
                                <p>
