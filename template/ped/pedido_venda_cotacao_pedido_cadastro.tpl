...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/2.1.1/jquery.min.js"></script>
    ...
    <div class="right_col" role="main">      
      <div class="">
        ...
        <form id="lancamento" data-parsley-validate  class="form-horizontal form-label-left" NAME="lancamento"  
            ...
            <input name=totalCredito        type=hidden value="{$totalCredito}">
                ...
                <div class="x_panel">
                  <div class="x_title">
                    ...
                    <ul class="nav navbar-right panel_toolbox">
                        <li><button type="button" class="btn btn-primary"  onClick="javascript:submitCadastraPedido({$id},'{$INTEGRAFIN}');">
                ...
                <div class="x_content small">
                    <div class="row">
                        ...
                        <div class="col-md-2 col-sm-2 col-xs-2 ">
                            ...
                            <div class="panel panel-default left_col">
                                <input class="form-control money" type="text" id="credito" name="credito" onBlur="preencheBonus()" value={$credito}>
                            </div>
                        </div>
                        {/if}
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-2 col-sm-2 col-xs-2">
                            ...
                            <div class="panel panel-default">
                                <select name="condPgto" class="form-control" onChange="javascript:submitAtualPedidoCondPG({$id});">
                                    {html_options values=$condPgto_ids selected=$condPgto_id output=$condPgto_names}
                                </select>
                            </div>
                        </div>
                        <div class="col-md-2 col-sm-2 col-xs-2">
                            <label for="centroCusto">Centro de Custo Entrega</label>
                            <div class="panel panel-default">
                                ...
                                <table id="datatable-buttons-1" class="table table-bordered jambo_table">
                                    <thead>
                                        <tr style="background: gray; color: white;">
                                            ...
                                            <th>Obs</th>
                                            <td style="width: 50px;"> <button type="button" class="btn btn-success btn-xs" onClick="javascript:submitAtualPedidoCondPGAcrescentar({$id},{$numParcelaAdd});"><span class="glyphicon glyphicon-plus" aria-hidden="true"></span></button> </td>
                                            <td style="width: 20px;"> <button type="button" class="btn btn-warning btn-xs" onClick="javascript:submitAtualPedidoCondPGAcrescentar({$id},{$numParcelaAdd - 2});"><span class="glyphicon glyphicon-minus" aria-hidden="true"></span></button> </td>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        ...
                                        {section name=i loop=$fin}
                                            ...
                                            <tr id="dados_financeiro" >
                                                <td> {$fin[i].PARCELA} </td>
                                                <td> 
                                                    <input {if $fin[i].SITPGTO eq "B"} disabled {elseif $fin[i]['TIPODOCTO_ID'] <> "N"} enabled {else} disabled {/if} class="form-control" type="text" id="venc" name="venc{$fin[i].PARCELA}" value={$fin[i].VENCIMENTO|date_format:"%d/%m/%Y"} >
                                                </td>
                                                <td> 
                                                    <input {if $fin[i].SITPGTO eq "B"} disabled {elseif $fin[i]['TIPODOCTO_ID'] <> "N"} enabled {else} disabled {/if} class="form-control" type="text" id="valor{$fin[i].PARCELA}" name="valor{$fin[i].PARCELA}" value={$fin[i].VALOR|number_format:2:",":"."}>

                                                </td>
                                                <td>
                                                    <select {if $fin[i].SITPGTO eq "B"} disabled {elseif $fin[i]['TIPODOCTO_ID'] <> "N"} enabled {else} disabled {/if} id="idTipoDoc" name="tipo{$fin[i].PARCELA}" class="form-control">
                                                        {if $fin[i]['TIPODOCTO_ID'] <> "N"}
                                                            {html_options values=$tipoDocto_ids selected=$tipoDocto_id output=$tipoDocto_names}
                                                        {else}
                                                            {html_options values=$tipoDocto_ids selected=$fin[i]['TIPODOCTO_ID'] output=$tipoDocto_names}
                                                        {/if}                                                    
                                                    </select>
                                                </td>
                                                <td>
                                                    <select {if $fin[i].SITPGTO eq "B"} disabled {/if} id="idConta" name="conta{$fin[i].PARCELA}" class="form-control">
                                                        {html_options values=$conta_ids selected=$conta_id output=$conta_names}
                                                    </select>
                                                </td>
                                                <td> 
                                                    <select {if $fin[i].SITPGTO eq "B"} disabled {elseif $fin[i]['TIPODOCTO_ID'] <> "N"} enabled {else} disabled {/if} id="idSitucao" name="situacao{$fin[i].PARCELA}" class="form-control">
                                                        {if $fin[i]['TIPODOCTO_ID'] <> "N"}
                                                            {html_options values=$situacaoLanc_ids selected=$situacaoLanc_id output=$situacaoLanc_names}
                                                        {else}
                                                            {html_options values=$situacaoLanc_ids selected='B' output=$situacaoLanc_names}
                                                        {/if}
                                                    </select>
                                                </td>
                                                <td colspan="3"> 
                                                    ...
                                                    <input {if $fin[i].SITPGTO eq "B"} disabled {/if} class="form-control" type="text" id="obs" name="obs{$fin[i].PARCELA}" value={$fin[i].OBS}>
    ...
    <script>
        function preencheBonus() {
            let valorCredito = document.getElementById("credito").value;
            document.getElementById("valor1").value = valorCredito;
        }
