<style>
      ...
      @media print {
            ...
            /* Remove sombras e efeitos para economia de tinta */
                  ...
                  <div class="col-md-5 col-sm-5 col-xs-5">
                        <div>
                              <h2 class="text-center">
                                    <strong>KARDEX SINTÉTICO</strong><br>
                              ...
                              <table class="table table-striped" style="margin-bottom: 0;">
                                    <thead>
                                          <tr>
                                                ...
                                                <th style="width: 10%; text-align: center;">Saída</th>
                                                <th style="width: 10%; text-align: center;">Saldo</th>
                                                <th style="width: 10%; text-align: center;">Centro de Custo</th>
                                          </tr>
                                    </thead>
                                    <tbody>
                                          {foreach $resultado as $item}
                                                <tr>
                                                      ...
                                                      <td>{$item.GRUPO}</td>
                                                      <td>{$item.CLIENTE}</td>
                                                      <td style="text-align: center;">{$item.ENTRADA|number_format:2:',':'.'}</td>
                                                      <td style="text-align: center;">{$item.SAIDA|number_format:2:',':'.'}</td>
                                                      <td style="text-align: center;">
                                                            {if $item.SALDO >= 0}
                                                                  <span class="saldo-positivo">{$item.SALDO|number_format:2:',':'.'}</span>
                                                            {else}
                                                                  <span class="saldo-negativo">{$item.SALDO|number_format:2:',':'.'}</span>
                                                            {/if}
                                                      </td>
...
<script type="text/javascript">
      function exportarTabelaParaExcel() {
            ...
            if (typeof converteColunaNumeroBR === 'function') {
                  converteColunaNumeroBR(ws, 4); // Entrada
                  converteColunaNumeroBR(ws, 5); // Saída
                  converteColunaNumeroBR(ws, 6); // Saldo
            }

            XLSX.utils.book_append_sheet(wb, ws, "Kardex Sintético");

            var dataIni = '{$dataIni}';
            var dataFim = '{$dataFim}';
            var nomeArquivo = 'Kardex_Sintetico_' + dataIni.replace(/\//g, '_') + '_a_' + dataFim.replace(/\//g, '_') + '.xlsx';
