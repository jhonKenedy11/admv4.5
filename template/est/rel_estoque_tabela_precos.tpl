                              ...
                              <table class="table table-striped" style="margin-bottom: 0;">
                                    <thead>
                                          <tr>
                                                ...
                                                <th style="width: 15%; text-align: center;">Preço Custo</th>
                                                <th style="width: 15%; text-align: center;">Preço Venda</th>
                                                <th style="width: 15%; text-align: center;">Margem</th>
                                          </tr>
                                    </thead>
                                    <tbody>
                                          {foreach $resultado as $item}
                                                <tr>
                                                      ...
                                                      <td style="text-align: center;">{$item.NOMEGRUPO}</td>
                                                      <td style="text-align: center;">R$ {$item.PRECO_CUSTO|number_format:2:',':'.'}</td>
                                                      <td style="text-align: center;">{if $item.PRECO_VENDA > 0}R$ {$item.PRECO_VENDA|number_format:2:',':'.'}{else}Não Informado{/if}</td>
                                                      <td style="text-align: center;">
                                                            {if $item.PRECO_CUSTO > 0}
                                                                  {math assign="margem" equation=(($item.PRECO_VENDA*100)/$item.PRECO_CUSTO)-100 format="%.2f"}
                                                                  {$margem}%
                                                            {else}
                                                                  -
                                                            {/if}
...
<script type="text/javascript">
      function exportarTabelaParaExcel() {
            ...
            XLSX.utils.book_append_sheet(wb, ws, "Tabela Preços");

            var dataImp = '{$dataImp}';
            var nomeArquivo = 'Tabela_Precos_' + dataImp.replace(/\//g, '_') + '.xlsx';
