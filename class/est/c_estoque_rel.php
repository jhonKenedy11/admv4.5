...
public function where_curva_ABC($sql){
    ...
    */ 
    ...
    $sql .= "GROUP BY I.ITEMESTOQUE, P.EMISSAO ";
...
public function select_estoque_nf($codProd, $ccusto, $dataIni, $dataFim, $saldoInicial = NULL){
    ...
    $sql = "SELECT NFP.CODPRODUTO, NFP.DESCRICAO,
        ...
        $sql .= " GROUP BY NFP.CODPRODUTO, NFP.DESCRICAO, CC.DESCRICAO";
        $banco = new c_banco();
        $banco->exec_sql($sql);
        $banco->close_connection();
...
public function select_estoque_ped($codProd, $ccusto, $dataIni, $dataFim, $saldoInicial = NULL){
    ...
    $sql = "SELECT PI.ITEMESTOQUE, PI.DESCRICAO, 
        ...
        if($saldoInicial == true){
         ...
         $sql .= " GROUP BY PI.ITEMESTOQUE, PI.DESCRICAO";
        $banco = new c_banco();
        $banco->exec_sql($sql);
        $banco->close_connection();
        return $banco->resultado;
...
public function select_relatorio_kardex_sintetico($par, $grupo){
   ...
   for ($i = 0; $i < count($result); $i++) {
        ...
        $entrada = $nfResult[0]['ENTRADA'] ?? 0;
        $nfSaida = $nfResult[0]['SAIDA'] ?? 0;
        $pedSaida = $pedResult[0]['SAIDA'] ?? 0;
        $saida = $nfSaida + $pedSaida;
        $saldo = $entrada - $saida;
        ...
        $saldoInicial = ($nfSaldoInicial[0]['ENTRADA'] ?? 0) - (($nfSaldoInicial[0]['SAIDA'] ?? 0) + ($pedSaldoInicial[0]['SAIDA'] ?? 0));
...
public function select_geral_localizacao($order = ''){
    ...
    $sql = "SELECT DISTINCT P.*, G.DESCRICAO AS NOMEGRUPO, 0 as ESTOQUE, 0 as RESERVA, LPAD(P.LOCALIZACAO, 5, '0') as TESTE ";
    $sql .= "FROM EST_PRODUTO P LEFT JOIN EST_GRUPO G ON (G.GRUPO=P.GRUPO) ";
    $sql .= "LEFT JOIN EST_PRODUTO_EQUIVALENCIA E ON (E.IDPRODUTO=P.CODIGO) ";
    $cond = strpos($sql, 'WHERE') === false ? 'WHERE' : 'AND';
    ...
    if ($this->m_par[3] != null) {
        ...
        if(isset($localizacao[1])){
            $sql .= "$cond (P.LOCALIZACAO >= '$localizacao[0]' AND P.LOCALIZACAO <= '$localizacao[1]' AND P.LOCALIZACAO <> '') ";
        }else{
            $sql .= "$cond (P.LOCALIZACAO IN ('$localizacao[0]') AND P.LOCALIZACAO <> '') ";
        }
    ...
    if ($order == 'grupo') {
        ...
        //utilizamos a função SUBSTRING para obter a letra e o número da coluna P.LOCALIZACAO separadamente. Em seguida, 
        //ordenamos primeiro pela letra (SUBSTRING(P.LOCALIZACAO, 1, 1)), depois pelo número convertido em um tipo numérico 
        //(CAST(SUBSTRING(P.LOCALIZACAO, 3) AS UNSIGNED)) e, por fim, pela descrição do produto (P.DESCRICAO).
 ...
 *   $par[6] = centrocusto
 */
public static function select_consulta_produto_preco($letra){
    ...
    $sqlS  = "SELECT NFP.CODIGONOTA, NFP.QUANT, NF.NUMERO, 'NF SAIDA' AS TIPO, NF.ORIGEM, NF.DOC, NF.EMISSAO, ";
    $sqlS .= "C.NOME, PI.QTSOLICITADA, NFP.UNITARIO, NFP.TOTAL, ";
    ...
    $sqlS  .= " PED.OS AS OS ,(PI.TOTAL - ((PI.TOTAL * PI.PERCDESCONTO)/100)) AS TOTALITEM, COALESCE(PRO.CUSTOREPOSICAO, 0) AS CUSTOREPOSICAO ";
    ...
    $sqlS  .= "LEFT JOIN FAT_PEDIDO PED ON (PED.ID = NF.DOC)  ";
    ...
    $sqlE  = "SELECT NFP.CODIGONOTA, NFP.QUANT ,NF.NUMERO, 'NF ENTRADA' AS TIPO, NF.ORIGEM, NF.DOC, NF.EMISSAO, ";
    $sqlE .= "C.NOME, CI.QTSOLICITADA, NFP.UNITARIO, NFP.TOTAL, ";
    ...
    $sqlE  .= " COALESCE(PECAS.OS, '') AS OS, (CI.TOTAL - ((CI.TOTAL * CI.PERCDESCONTO)/100) + NFP.VALORICMSST) AS TOTALITEM, COALESCE(NFP.CUSTOPRODUTO, 0) AS CUSTOREPOSICAO ";
    ...
    $sqlE  .= "LEFT JOIN (SELECT CODPRODUTO, NFENTRADA, MAX(CAT_ATENDIMENTO_ID) AS OS FROM CAT_AT_PECAS WHERE NFENTRADA IS NOT NULL GROUP BY CODPRODUTO, NFENTRADA) PECAS ON PECAS.CODPRODUTO = NFP.CODPRODUTO AND PECAS.NFENTRADA = NF.NUMERO ";
    ...
    $sqlPed = "SELECT PI.CODIGONOTA, PI.QTSOLICITADA , PED.ID, 'PEDIDO' AS TIPO, '' AS ORIGEM, '' AS DOC, PED.EMISSAO,  "; 
    $sqlPed .= "C.NOME, PI.QTSOLICITADA, PI.UNITARIO, PI.TOTAL, "; 
    ...
    $sqlPed .= " PED.OS AS OS , (PI.TOTAL - ((PI.TOTAL * PI.PERCDESCONTO)/100)) AS TOTALITEM, COALESCE(PRO.CUSTOREPOSICAO, 0) AS CUSTOREPOSICAO "; 
    ...
    $sql = "$sqlS UNION ALL $sqlE UNION ALL $sqlPed ORDER BY EMISSAO DESC";
    //echo strtoupper($sql);
    $banco = new c_banco;
    $banco->exec_sql($sql);
    $banco->close_connection();
...
public function select_relatorio_mov_estoque($letra, $order=''){
    ...
    $cond =  strpos($sqlPed, 'where') === false ? 'where' : 'and';
    $sqlPed .=  empty($dataIni) ? '':" $cond (PED.EMISSAO BETWEEN '".$dataIni." 00:00:00' AND '".$dataFim." 23:59:59') ";
