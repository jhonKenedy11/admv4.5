...
class c_manifesto_fiscal_sefaz extends c_user{
    ...
    public function downloadChaveAcesso($idNfe=null, $chave=null, $origem=null)
    {
        ...
        try {
            ...
            // Se tem resNFe mas não tem NFe completo, é um resumo
            if ($temResNFe && !$temNFeCompleto) {
                // Retorna um array especial indicando que é resumo
                // IMPORTANTE: Deve retornar array, não string, para ser detectado no controller
                return ['tipo' => 'resNFe', 'xml' => $xml, 'chave' => $chave];
            }

            // Verifica se é XML completo (deve ter estrutura NFe com infNFe)
            // Se não tem estrutura completa, pode ser resumo também
            $temInfNFe = (stripos($xml, '<infNFe') !== false);
            if (!$temInfNFe && $temResNFe) {
                // É resumo mesmo sem verificação de NFe
                return ['tipo' => 'resNFe', 'xml' => $xml, 'chave' => $chave];
            }
            ...
            //Nesse caso, ela percorre o XML e para cada correspondência encontrada, executa a função callback definida.
            $xmlProcessado = preg_replace_callback($padrao, function ($correspondencias) {
            ...
            // Verifica se o XML foi processado corretamente
            if (!empty($xmlProcessado)) {
                // Tenta inserir o XML no banco de dados
                $result = $this->insertXml($idNfe, $xmlProcessado);
                return $xmlProcessado;
            } else {
                // Retorna false em caso de erro no processamento
                return false;
            }
        } catch (\Exception $e) {
    ...
    public function obterDadosConsultaManifestoSefaz()
    {
        $banco = new c_banco();
        $banco->exec_sql(
            'SELECT ULTNSU, PROXIMACONSULTA, CSTAT FROM EST_MANIFESTO WHERE CENTROCUSTO = ' . (int) $this->m_empresacentrocusto
        ...
        if ($proximaCons === null
            ...
            return ['cStat' => 'ready', 'ultimaNSU' => $row['ULTNSU'] ?? null, 'message' => ''];
        }
