...
class c_conta extends c_user
{
    ...
    function getRegimeEspecialST()
    {
        return $this->regimeEspecialST;
    }
    function getRegimeEspecialSTMsg()
    {
        return $this->regimeEspecialSTMsg;
    }
    function getRegimeEspecialSTMT()
    {
        return $this->regimeEspecialSTMT;
    }
    ...
    function getRegimeEspecialSTMTAliq()
    {
        return $this->regimeEspecialSTMTAliq;
    }
    function getRegimeEspecialSTAliq()
    {
    ...
    function setIdAddress($id_address)
    {
    ...
    function setStatusObra($statusObra)
    {
    ...
    function setRegimeEspecialST($regimeEspecialST)
    {
        $this->regimeEspecialST = $regimeEspecialST;
    }
    function setRegimeEspecialSTMsg($regimeEspecialSTMsg)
    {
        $this->regimeEspecialSTMsg = $regimeEspecialSTMsg;
    }
    function setRegimeEspecialSTMT($regimeEspecialSTMT)
    {
        $this->regimeEspecialSTMT = $regimeEspecialSTMT;
    }
    function setContribuinteICMS($contribuinteICMS)
    {
        $this->contribuinteICMS = $contribuinteICMS;
    }
    function setConsumidorFinal($consumidorFinal)
    {
        $this->consumidorFinal = $consumidorFinal;
    }
    function setRegimeEspecialSTMTAliq($regimeEspecialSTMTAliq)
    {
        $this->regimeEspecialSTMTAliq = $regimeEspecialSTMTAliq;
    }
    function setRegimeEspecialSTAliq($regimeEspecialSTAliq)
    {
    ...
    {
        $this->desc_projeto = $descProjeto;
    }
    public function getDescProjeto()
    {
        return $this->desc_projeto;
    }

    public function setNumCno($numCno)
    {
        $this->num_cno = $numCno;
    }
    public function getNumCno()
    {
        return $this->num_cno;
    }

    public function setNumCrea($numCrea)
    {
        $this->num_crea = $numCrea;
    }
    public function getNumCrea()
    {
        return $this->num_crea;
    }
    ...
    public function where_pessoa_letra($letra)
    {
        ...
        if ($par[0] != '') {
            ...
            // verifica se é inicial, se não pesquisa por parte do nome
            if (strlen($par[0]) == 1) {
    ...
    //---------------------------------------------------------------
    //---------------------------------------------------------------
    public function incluiConta($origem = null)
    {
        ...
        if ($origem == 'manifesto') { //se a funcao que chamou e do form p_nfephp_40
    ...
    static public function selecionaCreditoUtilizadoCliente($cliente, $pedido)
    {
    ...
    static public function updateCreditoCliente($id, $pedidoutilizado = "", $valor = 0)
    {
     ...
     * @return Array id e nome co cliente ja existente
     */
    ...
    /**
     * 
     * @param String $cel celular do cliente
     * @return Array id e nome co cliente ja existente
     */
    ...
    /**
     * Funcao atualizar a classe do cliente
     * @name updateClasse
     * @param $cliente e $classe
     */
    public function updateClasse($cliente, $classe)
    {
    ...
    /**
     * Insere uma nova obra associada a um cliente na tabela 'fin_cliente_obra'.
     *
     * Os dados da obra, como ART, projeto, CNO, CREA, responsável técnico e o usuário que está inserindo,
     * são obtidos através dos métodos getters da classe e do atributo m_userid.
     *
     * @return mixed Retorna o status da operação de inserção no banco de dados.
     */
    public function incluiObra()
    {
        $banco = new c_banco;
        $sql = "INSERT INTO fin_cliente_obra (CLIENTE, ART, PROJETO, CNO, CREA, RESPONSAVEL_TECNICO, USERINSERT) VALUES (";
        $sql .= "'" . $this->getIdCliente() . "', ";
        $sql .= "'" . $this->getNumArt() . "', ";
    ...
    /**
     * Atualiza os dados de uma obra no banco de dados.
     *
     * Esta função executa uma query SQL para atualizar os campos ART, PROJETO, CNO, CREA,
     * RESPONSAVEL_TECNICO e USERCHANGE na tabela FIN_CLIENTE_OBRA, com base nos valores
     * definidos nos métodos getters do objeto atual. A atualização é feita para o registro
     * correspondente ao CLIENTE e ID da obra especificados.
     *
     * @return mixed Retorna o status da operação de atualização executada no banco de dados.
     */
    public function alteraObra()
    {
        $banco = new c_banco;
        $sql = "UPDATE FIN_CLIENTE_OBRA SET ";
        $sql .= "ART = '" . $this->getNumArt() . "', ";
    ...
    /**
     * Recupera uma lista de obras ativas associadas a um determinado cliente.
     *
     * @param int $clienteId O ID do cliente cujas obras devem ser recuperadas.
     * @return array Um array de obras, cada uma contendo os campos:
     *               - id
     *               - ART
     *               - PROJETO
     *               - CNO
     ...
     *               - RESPONSAVEL_TECNICO
     */
    public function selectObras($clienteId)
    {
        $banco = new c_banco;
        $sql = "SELECT id, ART, PROJETO, CNO, CREA, RESPONSAVEL_TECNICO, STATUS 
                FROM FIN_CLIENTE_OBRA 
                WHERE cliente = " . $clienteId . " ORDER BY STATUS = 'A' DESC, PROJETO ASC";
        $result = $banco->exec_sql($sql);
        $obras = array();
        if ($result) {
    ...
    /**
     * Funcao para gravar anexo das obras
     * @name gravaAnexoObra
     * @param String $mod
     * @param String $destaque
     * @return int id da imagem gravada
     */
    public function gravaAnexoObra($id, $path, $destaque)
    {
        $sql  = "INSERT INTO AMB_GED (`TABLE`, `TABLE_ID`, `PATH`, `DESTAQUE`, `USER_INSERT`) ";
        $sql .= "VALUES ('FIN_CLIENTE_OBRA'," . $id . ",'" . $path . "','" . $destaque . "', " . $this->m_userid . ")";
    ...
    /**
     * Exclui um anexo de obra específico pelo ID do anexo.
     *
     * Esta função executa uma query SQL para remover um anexo da tabela AMB_GED,
     * onde o ID do anexo corresponde ao parâmetro fornecido e a tabela associada é 'FIN_CLIENTE_OBRA'.
     *
     * @param int $id_anexo O ID do anexo a ser excluído.
     * @return bool Retorna o status da operação de exclusão executada no banco de dados.
     */
    public function excluiAnexoObra($id_anexo)
    {
        $sql = "DELETE FROM AMB_GED WHERE ID = " . $id_anexo . " AND `TABLE` = 'FIN_CLIENTE_OBRA'";
    ...
    /**
     * Seleciona anexos de uma obra específica.
     *
     * Esta função busca os anexos associados a uma obra identificada pelo ID fornecido.
     * Retorna um array com os dados dos anexos, incluindo ID, ID da obra, caminho completo e extensão do arquivo.
     *
     * @param int $id_obra O ID da obra para a qual os anexos devem ser selecionados.
     * @return array Um array contendo os dados dos anexos ou uma mensagem de erro em caso de falha.
     */
    public function selectAnexoObra($id_obra)
    {
        try {
            $banco = new c_banco();

            $sql  = "SELECT `ID`, `TABLE`, `TABLE_ID`, SUBSTRING_INDEX(`PATH`, '/', -4) AS `PATH`, ";
            $sql .= "SUBSTRING_INDEX(`PATH`, '.', -1) AS `EXTENSAO` ";
            $sql .= "FROM AMB_GED ";
            $sql .= "WHERE (`TABLE_ID` = " . (int)$id_obra . ") AND (`TABLE` = 'FIN_CLIENTE_OBRA')";

            $banco->exec_sql($sql);
            $anexos = $banco->resultado;
            $banco->close_connection();
            ...
            foreach ($anexos as $anexo) {
                $dadosFormatados[] = [
                    'id' => $anexo['ID'],
                    'id_obra' => $anexo['TABLE_ID'],
                    'extensao' => strtolower($anexo['EXTENSAO']),
                    'caminho_completo' => $anexo['PATH']
                ];
            }
            ...
            return [
                'success' => false,
                'message' => 'Erro ao carregar anexos: ' . $e->getMessage(),
                'sql' => $sql
            ];
        }
    }
    ...
    /**
     * Salva um anexo de obra
     * @param array $dados [id_obra, nameFile, tmp_dir, sizeFile]
     * @return array Resposta JSON formatada
     */
    public function salvarAnexoObra($dados)
    {
        $id_obra = (int)$dados['id_obra'];
        $nameFile = $dados['nameFile'];
        $tmp_dir = $dados['tmp_dir'];
        $sizeFile = $dados['sizeFile'];
        $anexoExt = strtolower(pathinfo($nameFile, PATHINFO_EXTENSION));

        if (!empty($nameFile) && is_uploaded_file($tmp_dir)) {
            // Define o diretório de upload
            $upload_dir = "/ged/fin_cliente_obra/" . $id_obra . "/";
            ...
            if (in_array($anexoExt, $valid_extensions)) {
                if ($sizeFile < 2000000) {
                    // Cria o path no formato cliente/ged/fin_cliente_obra/id/nomedoarquivo.extensão
                    $path = ADMraizCliente . $upload_dir . $nameFile;

                    // Grava o registro no banco primeiro
                    $idAnexo = $this->gravaAnexoObra($id_obra, $path, 'N');

                    if ($idAnexo) {
                        // Move o arquivo para o diretório
                        if (move_uploaded_file($tmp_dir, $upload_dir_full . $nameFile)) {
                            return [
                                'success' => true,
                                'id' => $idAnexo,
                                'caminho' => $path,
                            ...
                            // Se falhar ao mover, remove o registro do banco
                            $this->excluiAnexoObra($idAnexo);
                            return ['success' => false, 'message' => 'Falha ao mover arquivo'];
                        }
                    } else {
                        return ['success' => false, 'message' => 'Falha ao gravar no banco'];
                    }
                } else {
                    return ['success' => false, 'message' => 'Arquivo muito grande (máx. 2MB)'];
                }
            } else {
                return ['success' => false, 'message' => 'Extensão inválida (use JPEG, JPG ou PDF)'];
            }
        } else {
     ...
     * @param String pessoa
     * @return Array All columns of the table FIN_CLIENTE_ENDERECO
     * @version 03052023
     */
    public function search_customer_order($pessoa)
    {

        $sql = "SELECT * FROM fin_cliente_endereco ";
        $sql .= "WHERE CLIENTE = " . $pessoa . ";";
    ...
    /** 
     * @author Joshua Silva <joshuaatsilva@gmail.com>
     * @return Array All columns of the table FIN_CLIENTE_ENDERECO
     * @version 08052023
     */
    public function alter_address()
    {
        ...
        if (stripos($this->getEndereco(), 'AVENIDA')) {
            $tipo = 'AVENIDA';
        } else {
            $tipo = 'RUA';
        }

        $fone_contato = str_replace(array('_', '-'), '', $this->getContato());
        $fone = str_replace(array('_', '-'), '', $this->getFone());
        $cep = intval(str_replace('-', '', $this->getCep()));
        $status = $this->getStatus();

        $sql = "UPDATE fin_cliente_endereco SET";
        $sql .= " DESCRICAO = '" . $this->getObs() . "',";
        $sql .= " TIPOEND = '" . $tipo . "',";
        $sql .= " TITULOEND = '" . $this->getTitulo() . "',";
        $sql .= " ENDERECO = '" . $this->getEndereco() . "',";
        $sql .= " NUMERO = '" . $this->getNumero() . "',";
        ...
        $sql .= " FONE =  '" . $fone . "',";
        $sql .= " FONECONTATO =  '" . $fone_contato . "',";
        $sql .= " `STATUS` =  '" . $status . "' ";
        $sql .= " where ID = " . $this->getIdAddress();
