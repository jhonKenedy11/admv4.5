...
//Class c_pedidoVenda
Class c_pedidoVendaTelhas extends c_user {
    ...
    */
    ...
    public function setDesconto($desconto, $format=false) {
        ...
        $this->desconto = (($desconto == "0,00") or ($desconto == "0.00")) ? c_tools::stringToDouble($desconto) : $desconto;
...
//        endif;
    ...
    function setEnderecoEntrega($enderecoEntrega) { $this->enderecoEntrega = $enderecoEntrega;}    
    function getEnderecoEntrega() { 
        if(($this->enderecoEntrega == "") or ($this->enderecoEntrega == null)){
            return 'NULL';
        }else{
...
////// pedidor item não utilziado ===========================================    
    ...
    function getCentroCustoEntrega() { 
        if($this->centrocustoentrega == ''){
            $this->centrocustoentrega = $this->getCentroCusto();
        }
    ...
    public function setPedidoVenda() {
        ...
        $this->setCentroCustoEntrega($pedido[0]['CENTROCUSTOENTREGA']);
        ...
        $this->setCredito($pedido[0]['CREDITO']);
        $this->setPrazoEntrega($pedido[0]['PRAZOENTREGA']);
        $this->setMarkup($pedido[0]['MARKUP']);
        $this->setEnderecoEntrega($pedido[0]['ENDERECOENTREGA']);
    }
...
//        echo strtoupper($sql)."<BR>";
    ...
    */
     ...
     * @version 2000287
     */
    public function select_pedidoVenda_letra_old($letra, $motivos = null) {
        /*
         * [0] = data inicio
         * [1] = data FIm
         * [2] = cliente
        ...
        $sql = "SELECT p.*, D.PADRAO, C.NOMEREDUZIDO, C.NOME, C.USERLOGIN, V.NOMEREDUZIDO AS VENDEDOR, C.CIDADE, C.UF, E.NOMEFANTASIA AS DESCCUSTO, ";
        $sql .= "(SELECT COUNT(PED.ID) FROM FAT_PEDIDO PED WHERE PED.SITUACAO=5 AND PED.CLIENTE = P.CLIENTE)  AS COTACAO_CLIENTE ";
        $sql .= "FROM fat_pedido p ";
        ...
        if ($par[9] != ''){
            ...
            $sql .= empty($par[9]) ? '':" $cond (p.id  = ($par[9]))";
        }else{
            $cond =  strpos($sql, 'where') === false ? 'where' : 'and';
     ...
     */
        ...
        // Verifica se há produto especificado
        if ($par[3] != '') {
            $sql .= "INNER JOIN FAT_PEDIDO_ITEM I ON (I.ID=P.ID) AND (I.ITEMESTOQUE= " . $par[3] . ") ";
        }
        ...
        if (!empty($par[9])) {
            $conditions[] = "p.id = {$par[9]}";
        } else {
            if (!empty($par[0])) {
                $conditions[] = "p.emissao >= '$dataIni'";
            }
            if (!empty($par[1])) {
                $conditions[] = "p.emissao <= '$dataFim'";
            }
            if (!empty($par[2])) {
                $conditions[] = "p.cliente = {$par[2]}";
            }
            if (!empty($par[4])) {
                $conditions[] = "p.situacao IN ({$par[4]})";
            }
            if (!empty($par[5])) {
                $conditions[] = "p.usrfatura IN ({$par[5]})";
            }
            if (!empty($par[6])) {
                $conditions[] = "p.condpg IN ({$par[6]})";
            }
            if (!empty($par[7])) {
                $conditions[] = "p.ccusto IN ({$par[7]})";
            }
        }

        // Adiciona filtro de motivos, se existir
        if (!empty($par[8])) {
            $motivoArray = explode(",", $par[8]);
            $motivoCondition = [];
            foreach ($motivoArray as $motivo) {
                if (!empty($motivo)) {
                    $motivoCondition[] = "'$motivo'";
                }
            }
            if (!empty($motivoCondition)) {
                $conditions[] = "p.id IN (SELECT DISTINCT ID FROM FAT_PEDIDO_ITEM WHERE motivo IN (" . implode(',', $motivoCondition) . "))";
            }
        }

        $conditions[] = "p.situacao <> 8";
        if (!empty($conditions)) {
    ...
    public function forecast($dataIni,$dataFim,$qtdDiasUteis,$diasPassados,$where,$mes,$whereM, $ano ){
        ...
        $sql5 .= "from FIN_LANCAMENTO L where ".str_replace('p.ccusto','centrocusto',$where)." and (L.GENERO <> '1.40') AND (L.emissao >= '".$dataIni."') AND (L.emissao <= '".$dataFim."') and (L.TIPOLANCAMENTO = 'P') ";
        ...
        $sql9 .= "from FIN_LANCAMENTO L where ".str_replace('p.ccusto','centrocusto',$where)." and (L.GENERO <> '1.40') AND (L.emissao >= '".$dataIni."') AND ";
    ...
    public function totais($dataIni,$dataFim,$where,$wherel){ 
        ...
        $sql .= " (L.TOTAL > 0) and (L.GENERO <> '1.40')) as DESPESAS, ";
    ...
    public function metas($dataIni, $dataFim, $where, $vendedor=null){
        ...
        $sql  .= "LEFT JOIN amb_usuario u on (u.usuario=p.usrfatura) ";
        $sql  .= "where ".$where." and ((p.situacao = 6) or (p.situacao = 9) or (p.situacao = 13)) and (p.emissao >= '".$dataIni."') AND (p.emissao <= '".$dataFim."') ";
        if($vendedor !== null){
            if($vendedor !== ""){
                $sql .= " AND (p.usrfatura = '" . $vendedor . "') ";
            }
        }
        $sql  .= "group by u.nome order by u.nome asc"; 
    ...
    public function financeiro($dataIni, $dataFim, $where){ 
        ...
        $sql0 .= "and (L.TIPOLANCAMENTO = 'R') and (L.GENERO <> '1.40') AND ";
        ...
        $sql1 .= "where ".$where." AND (L.GENERO <> '1.40') AND (L.emissao >= '".$dataIni."') AND (L.emissao <= '".$dataFim."') and (L.TIPOLANCAMENTO = 'R') and ";
        ...
        $sql2 .= "and (L.TIPOLANCAMENTO = 'P') and (L.TOTAL > 0) AND (L.GENERO <> '1.40')";
...
// fim select_pedidoVenda_ClientePeriodo
    ...
    public function select_totalPedido() {
        ...
        if ($this->getId() != ''):
            $sql = "SELECT sum(qtsolicitada * unitario) as totalpedido ";
...
// fim select_pedidoVenda_ClientePeriodo
    ...
    public function incluiPedido() {
        ...
        // $sql .= "genero, ccusto, centrocustoentrega, odesferico, oeesferico, odcilindrico, oecilindrico, odeixo, oeeixo, odad, oead, medico, obs, credito, usrfatura, USERINSERT, DATEINSERT)";
        $sql .= "genero, ccusto, centrocustoentrega, obs, credito, enderecoentrega, usrfatura, USERINSERT, DATEINSERT)";
...
// fim incluiPedido
     ...
     * @return NULL quando ok ou msg erro
     */
    public function alteraPedidoSituacao($condPgto = null, $conn=null, $situacaoAnt = null){
        ...
        if (($this->getTotal() < $this->getCredito()) and
           ($this->getTotal() > 0) and ($this->getCredito() > 0)) {
            //verifica se o credito ja esta formatado
            if(mb_strpos($this->getCredito(), '.')){
    ...
    public function alteraPedido() {
        ...
        $sql = "cliente, pedido, situacao, emissao, entregador, idnatop, condpg, entradacondpg, ";
        $sql .= "desconto, total, moeda, contadeposito, especie, serie, horaemissao, ";
        $sql .= "genero, ccusto, obs, credito, enderecoentrega,USERINSERT, DATEINSERT)";
        ...
        $sql .= "situacao = '" . $this->getSituacao() . "', ";
        $sql .= "emissao = '" . $this->getEmissao('B') . "', ";
        if (($this->getDataEntrega('B') != "") and ($this->getDataEntrega() != null)){
            $sql .= "dataEntrega = '" . $this->getDataEntrega('B') . "', ";
        } else {
        ...
        $sql .= "serie = '" . $this->getSerie() . "', ";
        if (($this->getHoraEmissao('B') != "") and ($this->getHoraEmissao() != null)){
            $sql .= "horaemissao = '" . $this->getHoraEmissao('B') . "', ";
        } else {
            $sql .= "horaemissao = NULL, ";
        }            
        $sql .= "genero = '" . $this->getGenero() . "', ";
        ...
        $sql .= "obs = '" . $this->getObs() . "', ";
        $sql .= "credito = '" . $this->getCredito('B') . "', ";
        if (($this->getEnderecoEntrega('B') != "") and ($this->getEnderecoEntrega() != null)){
            $sql .= "enderecoentrega = " . $this->getEnderecoEntrega() . ", ";
        } else {
            $sql .= "enderecoentrega = NULL, ";
        } 
        //$sql .= "enderecoentrega = " . $this->getEnderecoEntrega() . ", ";
	    $sql .= "userchange = ".$this->m_userid.", ";
	    $sql .= "datechange = '".date("Y-m-d H:i:s")."' ";
...
*/
 ...
 //       echo strtoupper($sql) . "<BR>";
    ...
    public function atualizarFieldPedido($situacaoEmitirNf = '', $condPg = '', $obs = '', $genero = '', $idnatop = '', $entrega = '') {
        ...
        $sql .= ",USERCHANGE = " . $this->m_userid . " ";
        $sql .= ",DATECHANGE = CURRENT_TIMESTAMP() ";
        if($_REQUEST["submenu"] === "estorna" && $situacaoEmitirNf === '5'){ //se metodo que invocou for para estorno
            $sql .= ",USERCHANGE = NULL ";
            $sql .= ",DATECHANGE = NULL ";
        }else{
            $sql .= ",USERCHANGE = " . $this->m_userid . " ";
    ...
    public function alteraPedidoTotalTelhasDash() {
        ...
        $sql .= "Coalesce(Sum(total),0) as total, ";
        ...
        $banco->close_connection();
        if ($res > 0) {
            $lucroBruto = $res[0]['LUCROBRUTO'];
            $total = $res[0]['TOTAL'];
            $markup = ($lucroBruto / $total) *100;
    ...
    public function atualizarFieldPedidoNEW($situacaoEmitirNf ='', $condPg ='', $obs ='', $genero ='', $idnatop ='', $entrega ='', $alteraDataEmissao=null ,$conn=null) {
        $sql = "UPDATE fat_pedido ";
        $sql .= "SET pedido = '". $this->getId() ."' ";
        if ($situacaoEmitirNf != ''){
        ...
        // else {
        //     if  (($situacaoEmitirNf != '9') and ($situacaoEmitirNf != '11') and ($situacaoEmitirNf != '13')) {
        //         $sql .= ",EMISSAO = '".date("Y-m-d")."'  ";
        //         $sql .= ",HORAEMISSAO = '".date("H:i:s")."'  ";    
        //     }
        // } 
        if($alteraDataEmissao === true){
            $sql .= ",EMISSAO = '".date("Y-m-d")."' ";
            $sql .= ",HORAEMISSAO = '".date("H:i:s")."' ";
        }
    ...
    public function atualizaPedidoVenda($total) {
        ...
        $sql .= "CONDPG             = '".$this->getCondPg()."', ";
        $sql .= "CCUSTO             = '".$this->getCentroCusto()."', ";
        $sql .= "CENTROCUSTOENTREGA = '".$this->getCentroCustoEntrega()."', ";
        $sql .= "USRFATURA          = '".$this->getUsrFatura()."', ";
        $sql .= "EMISSAO            = '".date("Y-m-d")."', ";
        $sql .= "HORAEMISSAO        = '".date("H:i:s")."', ";
        $sql .= "PRAZOENTREGA       = '".$this->getPrazoEntrega()."', ";
    ...
    public function select_pedido_situacao(){
...
/**
* <b> É responsavel por inserir o crédito gerado(bônus) na tabela fin_cliente_credito </b>
* @name insereBonus
* @param (cliente, pedido, codigo do item, quantidade, valor unitario, valor total)
*/
    public function insereBonus($cliente, $pedido, $nritem, $quantidade, $unitario, $valor) {
        $sql = "INSERT INTO FIN_CLIENTE_CREDITO ";
        $sql .= "(CLIENTE, PEDIDO, NRITEM, QUANTIDADE, UNITARIO, VALOR, USERINSERT) VALUES ";
        $sql .= "('". $cliente."', '".$pedido."', '".$nritem."', '".$quantidade."' , '".$unitario."', '".$valor."' , '".$this->m_userid."')";
    ...
    /**
    * <b> É responsável por verificar bônus total disponível</b>
    * @name verificaDev
    * @param (pedido, codigo cliente)
    * @return array
    */
     ...
     */
    public function selectContaCliente($id) {
        $sql = "SELECT * ";
        ...
        return $banco->resultado;
          ...
          /*
     * @name selectVerificaCotacao
     * @param INT GetId codigo do cliente, chave primaria
     * @return string Coluna ID e PEDIDO
     * "0"		"DIGITAÇÃO"
        "1"		"SEPARAR"
        "2"		"CONFERIR"
        "3"		"EMITIR NF"
        ...
        "13"	"ENCOMENDA"
     */
    public function selectVerificaCotacao($client){
        $sql = "SELECT P.ID, P.PEDIDO, P.EMISSAO, P.TOTAL, E.NOMEFANTASIA AS CENTROCUSTO FROM FAT_PEDIDO P ";
        $sql .="INNER JOIN AMB_EMPRESA E ON P.CCUSTO = E.CENTROCUSTO ";
        $sql .="WHERE P.CLIENTE = '".$client."' AND P.SITUACAO = '5'; ";
    ...
    public function selectPedido($ped){
        $sql = "SELECT CLIENTE FROM FAT_PEDIDO ";
        $sql .="WHERE ID = '".$ped."'; ";
    ...
    public function selectPedidoAll($ped){
        $sql = "SELECT * FROM FAT_PEDIDO ";
        $sql .="WHERE ID = '".$ped."'; ";
    ...
    public function desaprovaPedido($id) {

        $sql = "UPDATE fat_pedido SET ";
        $sql .= "dataentrega = null, ";
        $sql .= "prazoentrega = '', ";       
        $sql .= "situacao = 0 ,";
        $sql .= "dataaprovacao = null, ";
        $sql .= "usraprovacao = null, ";
        $sql .= "userchange = " .$this->m_userid. ", ";
        $sql .= "datechange = CURRENT_TIMESTAMP() ";
        $sql .= "WHERE id = ".$id. ";";
        //echo strtoupper($sql)."<BR>";
    ...
    public function selectAllPed($data) {
        $dataIni = c_date::convertDateTxt($data[0]);
        $dataFim = c_date::convertDateTxt($data[1]);
        
        //remonta centro de custo
        $valores = explode(",", $data[3]);
        $valoresFormatados = [];
        ...
        $centrosCustos = implode(",", $valoresFormatados);
        //fim remonta

        $sql = "SELECT count(ID) as total from fat_pedido ";
        $sql .= "where emissao between '".$dataIni." 00:00:00' and '".$dataFim." 23:59:59' ";
        //verificar vendedor
        if((array_key_exists(2, $data)) || (array_key_exists(2, $data))){
            if($data[2] !== ''){
                $sql .=" and usrfatura = '".$data[2]."' ";
            }
        }

        if((array_key_exists(3, $data)) || (array_key_exists(3, $data))){
            if($data[3] !== ''){
                $sql .= " and ccusto in (".$centrosCustos.") ";
            }
        }
        //echo strtoupper($sql)."<BR>";
        ...
        $result = $banco->exec_sql($sql);
        $banco->close_connection();
        return $banco->resultado[0]['TOTAL'];
    }

    public function selectVerificaPedido($client){
        $sql = "SELECT P.PEDIDO, P.EMISSAO, P.HORAEMISSAO, P.FRETE, P.DESPACESSORIAS,
                       P.TOTAL, E.NOMEFANTASIA, U.NOMEREDUZIDO as VENDEDOR FROM FAT_PEDIDO P ";
        $sql .= "INNER JOIN AMB_EMPRESA E ON P.CCUSTO = E.CENTROCUSTO ";
        $sql .= "INNER JOIN AMB_USUARIO U ON P.USERINSERT = U.USUARIO ";
        $sql .= "WHERE P.CLIENTE = '".$client."' AND P.SITUACAO = '6' ORDER BY P.EMISSAO DESC;";
    ...
    public function selectPersonPedidosClient($id){
        $sql = 'SELECT p.PEDIDO, CONCAT(DATE_FORMAT(p.EMISSAO, "%d/%m/%Y")," - ", p.HORAEMISSAO) AS EMISSAO,';
        $sql .= 'a.PADRAO,Concat("R$ ",Replace(Replace(Replace(FORMAT(p.TOTAL, 2), ".", "|"), ",", "."), "|", ",")) AS TOTAL, e.NOMEFANTASIA AS CCUSTO ';
        $sql .= 'FROM FAT_PEDIDO p ';
        $sql .= 'INNER JOIN AMB_DDM a ON a.TIPO = p.SITUACAO '; 
        $sql .= 'INNER JOIN AMB_EMPRESA e ON e.CENTROCUSTO = p.CCUSTO ';
        $sql .= 'WHERE p.CLIENTE = '.$id.' AND p.SITUACAO <> 8 AND a.CAMPO = "SITUACAOPEDIDO" ORDER BY p.EMISSAO';
    ...
    public function select_endereco_id($id){
