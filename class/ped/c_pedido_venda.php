...
//        endif;
    ...
    function setNumeroOc($numeroOc) { $this->numeroOc = $numeroOc; }
...
////// pedidor item não utilziado ===========================================    
    ...
    function setDescontoItem($descontoItem, $format = false) {
        $this->descontoItem = $format ? $this->descontoItem = number_format($descontoItem, 2, ',', '.') : $descontoItem;
    }
    ...
    function getCentroCustoEntrega() { 
        if($this->centrocustoentrega == ''){
            $this->centrocustoentrega = $this->m_empresacentrocusto;
        }
    ...
    public function setPedidoVenda() {
        ...
        $this->setValorServicos($pedido[0]['VALORSERVICOS']);
    ...
    /**
     * Altera sequencia de um item em FAT_PEDIDO_ITEM
     * @name alteraSeqItem
     * @param idPedido
     * @param itemOrigem
     * @param itemDestino
     ...
     * @date 2022-06-01
     */
    public function alteraSeqItem($idPedido, $itemOrigem, $itemDestino){

        $banco = new c_banco;
        $sql = "update FAT_PEDIDO_ITEM  set nritem = 9999 where id=".$idPedido." and nritem=".$itemOrigem;
        $banco->exec_sql($sql);
        $banco->close_connection();

        if ($itemDestino < $itemOrigem){
            $sql = "update FAT_PEDIDO_ITEM  set nritem = nritem+1 where id=".$idPedido." ";
            $sql .= "and nritem>=".$itemDestino." and nritem<".$itemOrigem." ";
            $sql .= "order by nritem desc";
        }else{
            $sql = "update FAT_PEDIDO_ITEM  set nritem = nritem-1 where id=".$idPedido." ";
            $sql .= "and nritem>".$itemOrigem." and nritem<=".$itemDestino." ";
        }
        $banco = new c_banco;
        $banco->exec_sql($sql);
        $banco->close_connection();

        $sql = "update FAT_PEDIDO_ITEM  set nritem = ".$itemDestino." where id=".$idPedido." and nritem=9999";
        $banco = new c_banco;
        $banco->exec_sql($sql);
        $banco->close_connection();
...
//        echo strtoupper($sql)."<BR>";
    ...
    /**
     * Consulta para o Banco atraves do id
     * @name select_pedido_item_sem_codigo
     *
     * @return ARRAY  da table
     */
    public function select_pedido_item_sem_codigo($conn=null) {

        $sql = "SELECT NRITEM, DESCRICAO ";
        $sql .= "FROM FAT_PEDIDO_ITEM ";
        $sql .= "WHERE (ID = " . $this->getId() . " AND ITEMESTOQUE = 'SEM_CODIGO') ";
        //echo strtoupper($sql)."<BR>";
...
// fim select_pedidoVenda_ClientePeriodo
    ...
    public function select_total_com_desconto_pedido() {

        if ($this->getId() != ''):
            $sql = "SELECT sum(total-desconto) as totalpedido ";
            $sql .= "FROM fat_pedido_item ";
            $sql .= "WHERE (MOTIVO = 0) AND (id = " . $this->getId() . ") ";
            //echo strtoupper($sql)."<BR>";
...
// fim select_pedidoVenda_ClientePeriodo
    ...
    public function incluiPedido() {
        ...
        $sql .= "genero, ccusto, centrocustoentrega, obs, credito, usrfatura, USERINSERT, DATEINSERT)";
...
//            $sql .= "(descricao like '%" . $par[0] . "%') ";
    ...
    public function pedido_venda_item($select=true, $arr='') {
        ...
        $this->setCusto($item[0]['CUSTO']);
        $this->setMargemLiquida($item[0]['MARGEMLIQUIDA']);
...
*/
 ...
 //       echo strtoupper($sql) . "<BR>";
    ...
    public function duplicaPedido($conn=null) {
        ...
        $sqli .= "TAXAENTREGA,TOTALRECEBIDO,GENERO,CCUSTO,TIPOENTREGA,TABELAPRECO,IPI,CONTATO, ";
    ...
    public function alteraPedidoItemTelhas($conn=null) {
        ...
        $sql .= "percdesconto = " . $this->getPercDesconto('B') . ", ";
        $sql .= "total = " . $this->getTotalItem('B') . ", ";
        $sql .= "grupoestoque = '" . $this->getGrupoEstoque() . "', ";
        $sql .= "descricao = '" . $this->getDescricaoItem() . "', ";
        $sql .= "precopromocao = " . $this->getPrecoPromocao() . ", ";
    ...
    public function atualizaPedidoVenda($total='') {
        ...
        $sql .= "SITUACAO           = '".$this->getSituacao()."', ";
        $sql .= "CONDPG             = '".$this->getCondPg()."', ";
        $sql .= "CCUSTO             = '".$this->m_empresacentrocusto."', ";
    ...
    public function atualizaPedidoVendaItem() {
        $sql = "UPDATE FAT_PEDIDO_ITEM ";
        $sql .= "SET ITEMFABRICANTE         = '". $this->getItemFabricante() ."',  ";
        ...
        $sql .= "WHERE (ID = ".$this->getId()." AND NRITEM = '" . $this->getNrItem() . "');";                        
        $banco = new c_banco;
        $banco->exec_sql($sql);
        $banco->close_connection();
    }
    ...
    public function select_nota_fiscal($id){
        $sql = "SELECT n.ID, n.MODELO, n.SERIE, n.NUMERO, n.PESSOA, n.EMISSAO, n.IDNATOP,  
                       n.NATOPERACAO, n.TIPO, n.SITUACAO, n.FORMAPGTO, n.DATASAIDAENTRADA,  
                       n.FINALIDADEEMISSAO, n.CENTROCUSTO, n.CONDPGTO, n.GENERO, n.TOTALNF, n.ORIGEM, 
                       n.FRETE, n.DESPACESSORIAS, n.USRFATURA, n.DESCONTOGERAL, c.NOME  FROM EST_NOTA_FISCAL n ";
    ...
    public function selectPersonPedidosClient($id){
        $sql = 'SELECT p.PEDIDO, CONCAT (DATE_FORMAT(p.EMISSAO, "%d/%m/%Y"),IFNULL(CONCAT(" - ", TIME_FORMAT(p.HORAEMISSAO, "%H:%i:%s")), "")) AS EMISSAO,';
